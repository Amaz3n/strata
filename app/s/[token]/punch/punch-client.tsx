...
import { completeSubPortalPunchItemAction } from "./actions"
...
export function PunchClient({ punchItems, token }: PunchClientProps) {
  ...
  if (items.length === 0) {
    return (
      <div className="border border-border bg-card px-4 py-12 text-center">
        <CheckSquare className="mx-auto mb-3 size-8 text-muted-foreground" />
        <p className="text-sm font-medium">No punch items assigned to you</p>
        <p className="mt-1 text-sm text-muted-foreground">
          Punch work appears here when the builder assigns it to your company.
        </p>
      </div>
    )
  }
