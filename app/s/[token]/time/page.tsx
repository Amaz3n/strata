...
export default async function PortalTimePage({ params }: Props) {
  ...
  if (!shell.features.showTime) notFound()
  ...
  return (
    <>
      <PortalPageHeader
        title="Submit time"
        description="Log hours your crew worked on this project."
      />
      ...
      <div className="border border-border bg-card p-4 sm:p-6">
        <form action={submitPortalTimeAction.bind(null, token)} className="space-y-4" encType="multipart/form-data">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div className="space-y-2">
              <Label>Burden</Label>
              <Input name="burden_multiplier" type="number" min="1" step="0.01" defaultValue="1" className="tabular-nums" />
            </div>
          </div>
          <div className="space-y-3">
            <Label>Crew lines</Label>
            {Array.from({ length: 5 }).map((_, index) => (
              <div key={index} className="grid gap-2 border border-border p-2 sm:grid-cols-[1.2fr_0.7fr_0.8fr_1.2fr]">
                <Input name="worker_name" placeholder={index === 0 ? "Worker name" : "Optional worker"} required={index === 0} />
                <Input name="hours" type="number" step="0.25" min="0.25" max="24" placeholder="Hours" required={index === 0} className="tabular-nums" />
                <Input name="base_rate" inputMode="decimal" placeholder="Rate" required={index === 0} className="tabular-nums" />
