...
function MeetingItemEditor({ projectId, item, pending, submit }: { projectId: string; item: MeetingItem; pending: boolean; submit: (work: () => Promise<void>) => void }) {
  const router = useRouter()
  const [editing, setEditing] = useState(false)
  if (!editing) return <div className="grid grid-cols-[52px_1fr_auto] gap-3 border-b py-3 last:border-0"><span className="font-mono text-xs">{item.item_number}</span><div><div className="font-medium">{item.topic}</div>{item.discussion ? <p className="mt-1 whitespace-pre-wrap text-sm text-muted-foreground">{item.discussion}</p> : null}<div className="mt-2 flex flex-wrap gap-3 text-xs text-muted-foreground"><span className="capitalize">{item.status}</span><span>BIC: {item.ball_in_court ?? "—"}</span><span>Due: {item.due_date ?? "—"}</span></div></div><div className="flex items-start gap-1"><Button size="sm" variant="ghost" disabled={pending} onClick={() => setEditing(true)}>Edit</Button>{item.task_id ? <Badge variant="outline">Task linked</Badge> : <Button size="sm" variant="ghost" disabled={pending} onClick={() => submit(async () => { unwrapAction(await createMeetingItemTaskAction(projectId, { meeting_item_id: item.id })); toast.success("Task created"); router.refresh() })}>Create task</Button>}</div></div>
  return <form className="grid gap-2 border-b py-3" onSubmit={(event) => { event.preventDefault(); const element = event.currentTarget; const form = new FormData(element); submit(async () => { unwrapAction(await updateMeetingItemAction(projectId, item.id, { topic: form.get("topic"), discussion: form.get("discussion") || null, status: form.get("status"), ball_in_court: form.get("ball_in_court") || null, due_date: form.get("due_date") || null })); setEditing(false); toast.success("Meeting item updated"); router.refresh() }) }}><div className="grid gap-2 md:grid-cols-[1fr_140px]"><Input name="topic" defaultValue={item.topic} required /><Select name="status" defaultValue={item.status}><SelectTrigger><SelectValue /></SelectTrigger><SelectContent><SelectItem value="open">Open</SelectItem><SelectItem value="closed">Closed</SelectItem><SelectItem value="info">Information</SelectItem></SelectContent></Select></div><Textarea name="discussion" defaultValue={item.discussion ?? ""} placeholder="Discussion / decision notes" /><div className="grid gap-2 md:grid-cols-2"><Input name="ball_in_court" defaultValue={item.ball_in_court ?? ""} placeholder="Ball in court" /><Input name="due_date" type="date" defaultValue={item.due_date ?? ""} /></div><div className="flex gap-2"><Button type="submit" size="sm" disabled={pending}>Save item</Button><Button type="button" size="sm" variant="ghost" onClick={() => setEditing(false)}>Cancel</Button></div></form>
}
