...
import { TradeLookaheadClient } from "@/components/schedule/trade-lookahead-client"
...
export default async function TradeLookaheadPage({ searchParams }: { searchParams: Promise<{ weeks?: string }> }) {
  ...
  return (
    <PageLayout
      title="Trade look-aheads"
      breadcrumbs={[{ label: "Schedule", href: "/schedule" }, { label: "Trade look-aheads" }]}
    >
      <div className="space-y-3 p-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <p className="text-sm text-muted-foreground">
            Upcoming work grouped by trade company. Sending is deduplicated once per company per day.
          </p>
          <div className="flex border text-xs" role="group" aria-label="Look-ahead window">
            {WINDOWS.map((window) => (
              <Link
                key={window}
                href={window === 3 ? "/schedule/trades" : `/schedule/trades?weeks=${window}`}
                className={cn(
