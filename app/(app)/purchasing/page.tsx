...
export default async function PurchasingPage() {
  ...
  const [agreements, health, bids, exceptions, vpos, completions, variance, companies, costCodes, communities, plans] = await Promise.all([
    listPriceAgreements({ page: 1, pageSize: 50 }), getPriceBookHealth(), listOrgBidPackages(),
    listPoExceptions({ status: "open", page: 1, pageSize: 50 }), listVarianceOrders({ status: "pending", page: 1, pageSize: 50 }),
    listPoCompletions({ page: 1, pageSize: 50 }), getVarianceAnalysis({ startDate: monthStart, endDate: today.toISOString().slice(0, 10) }),
