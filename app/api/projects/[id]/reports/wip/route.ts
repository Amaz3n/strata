...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const report = await getProjectWipOverUnderReport({ projectId, asOf })

    if (format === "csv") {
      const csv = toCsv(report.rows, columns)
      const filename = `wip-over-under-${projectId}-${report.as_of}.csv`
    ...
    return NextResponse.json(
      { as_of: null, project_id: projectId, rows: [], error: error?.message ?? "Failed to build WIP report" },
      { status: 200 },
    )
  }
}
