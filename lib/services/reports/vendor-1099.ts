...
export async function getVendor1099Report({ year, orgId }: { year?: number; orgId?: string } = {}): Promise<Vendor1099Report> {
  ...
  const [companiesResult, allocationsResult, billsResult, expensesResult] = await Promise.all([
    ...
    supabase.from("project_expenses").select("vendor_company_id, amount_cents, tax_cents, expense_date, status").eq("org_id", resolvedOrgId).not("vendor_company_id", "is", null).gte("expense_date", `${taxYear}-01-01`).lte("expense_date", `${taxYear}-12-31`).in("status", ["approved", "locked"]),
  ])
