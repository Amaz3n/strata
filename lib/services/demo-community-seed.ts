...
export async function seedSampleCommunity(orgId: string, actorUserId: string): Promise<{ communityId: string }> {
  ...
  const takeoffRows = SAMPLE_COMMUNITY_SPEC.plans.flatMap((plan) => costCodes.map((costCode, index) => ({ plan_code: plan.code, elevation_code: "", cost_code: costCode.code, description: `Sample scope ${index + 1}`, quantity: index % 4 === 0 ? plan.heatedSqft : 1, uom: index % 4 === 0 ? "sf" : "ls", unit_cost_cents: index % 4 === 0 ? 125 : 3500 + index * 275 })))
  ...
  const optionRows = Array.from({ length: 15 }, (_, index) => ({ category: `Cypress Sample — ${["Kitchen", "Flooring", "Electrical", "Exterior"][index % 4]}`, option_code: `CYP-OPT-${String(index + 1).padStart(2, "0")}`, option_name: ["Cabinet upgrade", "Flooring upgrade", "Lighting package", "Exterior detail"][index % 4] + ` ${Math.floor(index / 4) + 1}`, scope: index < 5 ? "structural" : "design_studio", price_cents: 75000 + index * 12500, cost_cents: 42000 + index * 8000, cost_code: costCodes[index % costCodes.length].code, lead_time_days: 14 + index, is_default: index % 5 === 0, applicable_plans: SAMPLE_COMMUNITY_SPEC.plans.map((plan) => plan.code).join(";") }))
  ...
  const priceRows = Array.from({ length: 30 }, (_, index) => ({ vendor: `Cypress Sample Trade ${index % 6 + 1}`, cost_code: costCodes[index % costCodes.length].code, description: `Sample agreement ${index + 1}`, uom: index % 3 === 0 ? "sf" : "ls", unit_price_cents: index % 3 === 0 ? 150 + index * 5 : 25000 + index * 1250, plan_code: SAMPLE_COMMUNITY_SPEC.plans[index % 3].code, effective_start: new Date().toISOString().slice(0, 10) }))
  ...
  const budgets = houseLots.flatMap((lotNumber) => costCodes.slice(0, 5).map((costCode, index) => ({ community: SAMPLE_COMMUNITY_SPEC.name, lot_number: String(lotNumber), cost_code: costCode.code, budget_cents: 5000000 + index * 1000000 })))
  const purchaseOrders = houseLots.flatMap((lotNumber) => [0, 1].map((index) => ({ community: SAMPLE_COMMUNITY_SPEC.name, lot_number: String(lotNumber), po_number: `CYP-${lotNumber}-${index + 1}`, vendor: `Cypress Sample Trade ${index + 1}`, cost_code: costCodes[index].code, description: index ? "Open finish allowance" : "Open framing balance", remaining_cents: 1250000 + index * 500000, original_cents: 2500000 + index * 500000 })))
  const houses = houseLots.map((lotNumber, index) => ({ community: SAMPLE_COMMUNITY_SPEC.name, lot_number: String(lotNumber), plan_code: SAMPLE_COMMUNITY_SPEC.plans[index % 3].code, elevation_code: index % 2 ? "A" : "B", stage_task: ["Foundation", "Framing", "Dry-in", "MEP rough", "Drywall", "Interior finishes"][index], stage_date: asOfDate, budget_total_cents: 35000000, sold: index >= 4, buyer_name: index >= 4 ? `Sample Buyer ${index - 3}` : "", buyer_email: index >= 4 ? `sample-buyer-${index - 3}@example.invalid` : "", sale_price_cents: index >= 4 ? 52500000 + index * 1000000 : "", sale_date: index >= 4 ? asOfDate : "" }))
