...
import type { BillingCycleItem } from "@/lib/financials/billing-cycle-summary"
...
export type FinancialsReviewQueueData = Awaited<ReturnType<typeof loadCostInboxData>>

/**
 * Flatten the queue into the shape `summarizeBillingCycle` reads, so the close
 * workflow and the Cost Inbox header report the cycle from one definition.
 */
export function toBillingCycleItems(queue: {
  timeEntries: any[]
  expenses: any[]
  vendorBills: any[]
  openCosts: any[]
  ...
  const codesEnabled = queue.costCodesEnabled ?? true
  const base = (row: any, kind: string, amountCents: number, isBillableCost: boolean): BillingCycleItem => ({
    id: isBillableCost ? String(row.id) : `${kind}:${row.id}`,
    state: (row.queue_state ?? "needs-review") as BillingCycleItem["state"],
    amountCents,
    ageDays: ageDays(row.__date),
    needsCostCode: codesEnabled && !isBillableCost && !row.cost_code_id,
    needsReceipt: row.proof_complete === false,
    needsRate: false,
    isBillableCost,
    billingPeriodId: row.billing_period_id ?? null,
    lateToBillingPeriodId: row.late_to_billing_period_id ?? null,
  })

  return [
    ...queue.timeEntries.map((row) => ({
      ...base({ ...row, __date: row.work_date }, "time", Number(row.cost_cents ?? 0), false),
      needsRate: Number(row.base_rate_cents ?? 0) <= 0,
    })),
    ...queue.expenses.map((row) =>
      base(
        { ...row, __date: row.expense_date },
        "expense",
        Number(row.amount_cents ?? 0) + Number(row.tax_cents ?? 0),
        false,
      ),
    ),
    ...queue.vendorBills.map((row) => ({
      ...base({ ...row, __date: row.bill_date ?? row.due_date }, "vendor_bill", Number(row.total_cents ?? 0), false),
      // A bill is coded only when every line carries a cost code.
      needsCostCode:
        codesEnabled && !((row.actual_lines ?? []).length > 0 && (row.actual_lines ?? []).every((line: any) => Boolean(line.cost_code_id))),
    })),
    ...queue.openCosts.map((row) =>
      base({ ...row, __date: row.occurred_on }, "billable_cost", Number(row.billable_cents ?? 0), true),
    ),
  ]
}
...
export async function loadCostInboxData(projectId: string) {
