...
export type { BankMatchCandidate, BankMatchSuggestion } from "@/lib/services/books/bank-match-rules"
...
export type BankReviewRow = {
   ...
   */
  ruleSuggestion:
    | { glAccountId: string; accountCode: string; accountName: string; ruleId: string; confidence: number; autoApplies: boolean }
    | null
}
 ...
 */
export async function reviewUnmatchedBankTransactions(input: {
  ...
  let accountQuery = service
    ...
    .eq("org_id", context.orgId)
  if (input.bankAccountId) accountQuery = accountQuery.eq("id", input.bankAccountId)
  const { data: accountData, error: accountError } = await accountQuery
  if (accountError) throw new Error(`Failed to load bank accounts: ${accountError.message}`)
  const accounts = z
    .array(z.object({ id: z.string().uuid(), name: z.string(), gl_account_id: z.string().uuid().nullable() }))
    .parse(accountData ?? [])
  ...
  let transactionQuery = service
    ...
    .range(0, REVIEW_TRAY_CAP)
  if (input.bankAccountId) transactionQuery = transactionQuery.eq("bank_account_id", input.bankAccountId)
  const { data: transactionData, error: transactionError } = await transactionQuery
  if (transactionError) throw new Error(`Failed to load bank transactions: ${transactionError.message}`)
  ...
  const transactions = z
    .array(
      z.object({
        ...
        matches: z.array(z.object({ matched_amount_cents: z.number().int().nullable(), status: z.string() })),
      }),
    )
    .parse(transactionData ?? [])
    // Same definition of "unmatched" the workspace uses: confirmed matches that
    // do not add up to the transaction still leave it open.
    .filter((transaction) => {
      const matched = transaction.matches
        .filter((match) => match.status === "confirmed")
        .reduce((sum, match) => sum + Number(match.matched_amount_cents ?? 0), 0)
      return matched < transaction.amount_cents
    })

  const truncated = transactions.length > REVIEW_TRAY_CAP
  const page = truncated ? transactions.slice(0, REVIEW_TRAY_CAP) : transactions

  const mappedGlAccountIds = Array.from(
    new Set(
      page
        .map((transaction) => accountById.get(transaction.bank_account_id)?.gl_account_id)
    ...
    mappedGlAccountIds.length > 0 && dates.length > 0
      ? z.array(candidateSchema).parse(
          (
            await service
              .from("journal_lines")
              .select("id, account_id, debit_cents, credit_cents, description, entry:journal_entries!inner(entry_date, status)")
              .eq("org_id", context.orgId)
              .in("account_id", mappedGlAccountIds)
              .eq("entry.status", "posted")
              .gte("entry.entry_date", shiftDays(dates[0], -MATCH_WINDOW_DAYS))
              .lte("entry.entry_date", shiftDays(dates[dates.length - 1], MATCH_WINDOW_DAYS))
              .limit(2000)
          ).data ?? [],
        )
      : []
  ...
  const glAccountById = new Map(
    z.array(z.object({ id: z.string().uuid(), code: z.string(), name: z.string() }))
  ...
  const { data: takenData } = await service
    ...
    .eq("org_id", context.orgId)
    .eq("status", "confirmed")
  const taken = new Set((takenData ?? []).map((row) => String(row.journal_line_id)))
  ...
  const rows: BankReviewRow[] = page.map((transaction) => {
    ...
    if (!account?.gl_account_id) {
      return { ...base, best: null, alternatives: [], accountUnmapped: true, ruleSuggestion: null }
    }
...
export type BankReviewTray = Awaited<ReturnType<typeof reviewUnmatchedBankTransactions>>
