...
import {
  buildMirrorSummary,
  mirrorReference,
} from "@/lib/services/books/mirror-rules";
...
export async function getExternalAccountMappingWorkspace(input: {
  connectionId: string;
  orgId?: string;
}) {
  const context = await requireMirrorContext(input.orgId);
  const service = createServiceSupabaseClient();
  const [connectionResult, accountsResult, mappingsResult] = await Promise.all([
    service
    ...
    service
      ...
      .select("gl_account_id, external_account_id, external_account_name")
      .eq("org_id", context.orgId)
  ...
  if (connectionResult.data.status !== "active")
    throw new Error("Reconnect this accounting provider before mapping accounts");
  if (!isAccountingProviderKey(connectionResult.data.provider))
    throw new Error(`Unsupported accounting provider: ${connectionResult.data.provider}`);
  if (accountsResult.error)
    throw new Error(`Failed to load Arc accounts: ${accountsResult.error.message}`);
  if (mappingsResult.error)
    throw new Error(`Failed to load account mappings: ${mappingsResult.error.message}`);

  const provider = getProvider(connectionResult.data.provider);
  const externalAccounts = provider.listAllAccounts
    ? await provider.listAllAccounts({ connectionId: input.connectionId })
    : [];
  return {
    connection: connectionResult.data,
    arcAccounts: accountsResult.data ?? [],
    externalAccounts,
...
export async function saveExternalAccountMappings(input: {
  ...
  const { data: accounts, error: accountError } = await service
    ...
    .in(
      "id",
      input.mappings.map((mapping) => mapping.glAccountId),
    );
  if (
    accountError ||
    (accounts?.length ?? 0) !==
      new Set(input.mappings.map((mapping) => mapping.glAccountId)).size
  )
    throw new Error(
      "One or more GL accounts do not belong to this organization",
    );
  const { error } = await service.from("accounting_account_mappings").upsert(
    input.mappings.map((mapping) => ({
      org_id: context.orgId,
      connection_id: input.connectionId,
      gl_account_id: mapping.glAccountId,
      external_account_id: mapping.externalAccountId,
      external_account_name: mapping.externalAccountName ?? null,
      mapping_source: "manual",
      approved_by: context.userId,
      approved_at: new Date().toISOString(),
    })),
    { onConflict: "org_id,connection_id,gl_account_id" },
  );
  if (error)
    throw new Error(
      `Failed to save external account mappings: ${error.message}`,
    );
  await Promise.all([
    recordEvent({
      orgId: context.orgId,
      actorId: context.userId,
      eventType: "books.external_account_mappings_saved",
      entityType: "accounting_connection",
      entityId: input.connectionId,
      payload: { mapping_count: input.mappings.length },
    }),
    recordAudit({
      orgId: context.orgId,
      actorId: context.userId,
      ...
      entityType: "accounting_connection",
      entityId: input.connectionId,
      after: { account_mapping_count: input.mappings.length },
      source: "books.external-mirror",
    }),
  ]);
 ...
 * summarized journal.
 *
 * Per-source-transaction pushes turn the external system into an
 ...
 * posted.
 */
export async function mirrorPeriodSummary(input: {
  periodId: string;
  connectionId: string;
  orgId?: string;
}) {
  ...
  const [settingsResult, connectionResult, periodResult] = await Promise.all([
    ...
    service
      .from("accounting_connections")
      .select("id, provider, status, label")
      .eq("org_id", context.orgId)
      .eq("id", input.connectionId)
      .single(),
    service
      ...
      .select(
        "id, period_start, period_end, status, fiscal_year, fiscal_period",
      )
      .eq("org_id", context.orgId)
  ...
  if (
    ...
    settingsResult.data.ledger_authority !== "arc" ||
    settingsResult.data.external_sync_posture !== "outbound_mirror"
  ) {
    throw new Error(
      "This organization is not in Arc-authoritative external-mirror mode",
    );
  }
  if (
    connectionResult.error ||
    connectionResult.data.status !== "active" ||
    !isAccountingProviderKey(connectionResult.data.provider)
  ) {
  ...
  const provider = getProvider(connectionResult.data.provider);
  if (
    !provider.capabilities.supportsJournalEntryPush ||
    !provider.pushSummaryJournal
  ) {
    throw new Error(
      `${connectionResult.data.label} cannot accept a summarized journal`,
    );
  }
  ...
  if (entriesError)
    throw new Error(`Unable to load period journals: ${entriesError.message}`);
  const entryIds = (entries ?? []).map((entry) => entry.id);
  if (entryIds.length === 0)
    throw new Error("This period has no posted journals to mirror");
  ...
  if (linesResult.error)
    throw new Error(
      `Unable to load period journal lines: ${linesResult.error.message}`,
    );
  if (accountsResult.error)
    throw new Error(
      `Unable to load the chart of accounts: ${accountsResult.error.message}`,
    );
  if (mappingsResult.error)
    throw new Error(
      `Unable to load external account mappings: ${mappingsResult.error.message}`,
    );
  ...
  const summary = buildMirrorSummary({
    ...
    mappings: (mappingsResult.data ?? []).map((mapping) => ({
      ...
      externalAccountName:
        (mapping.external_account_name as string | null) ?? null,
    })),
    periodLabel,
  });
  ...
  if (!summary.ok) {
    ...
    if (summary.unmappedAccounts.length > 0) {
      const names = summary.unmappedAccounts
        .map((account) => `${account.code} ${account.name}`)
        .join(", ");
      throw new Error(
        `Map these accounts to ${connectionResult.data.label} before mirroring: ${names}`,
      );
    }
    throw new Error(
      `The period summary does not balance (off by ${summary.unbalancedBy} cents)`,
    );
  }
  ...
  if (recordError)
    throw new Error(
      `Failed to record the mirrored period: ${recordError.message}`,
    );
