...
async function resolveBooksVersions(orgId: string) {
  ...
  if (settings.error)
    throw new Error(`Failed to load Books settings: ${settings.error.message}`);
  return {
...
async function requirePeriodPermission(
  permission: "books.close" | "books.reopen",
  orgId?: string,
) {
...
export async function runBooksCloseChecklist(periodId: string, orgId?: string) {
  ...
  const { data: periodData, error: periodError } = await service
    ...
    .single();
  if (periodError)
    throw new Error(`Failed to load accounting period: ${periodError.message}`);
  ...
  const [
    ...
    retainageMovement,
    journalProposals,
    unassignedTaxInvoices,
    unassignedUseTaxBills,
    fixedAssets,
    ...
    collectPages(
      (from, to) =>
        service
          ...
          .eq("org_id", context.orgId)
          .eq("status", "closed")
          .lte("statement_end", period.period_end)
          .order("created_at", { ascending: true })
          .order("id", { ascending: true })
          .range(from, to),
    ...
    collectPages(
      (from, to) =>
        service
          ...
          .select("id, bank_transaction_matches(status)")
          .eq("org_id", context.orgId)
    ...
    collectPages(
      (from, to) =>
        service
          .from("companies")
          .select("id, tax_id_last4, w9_file_id, is_1099_eligible")
          .eq("org_id", context.orgId)
    ...
    service
      ...
      .select("id", { count: "exact", head: true })
      .eq("org_id", context.orgId)
      .gt("use_tax_accrued_cents", 0)
      .is("tax_jurisdiction_id", null)
      .lte("bill_date", period.period_end)
      .in("status", [...PAYABLE_VENDOR_BILL_STATUSES]),
    service
      .from("books_fixed_assets")
      .select("id,name,placed_in_service_on,status")
      .eq("org_id", context.orgId)
      .in("status", ["active", "fully_depreciated"])
      .lte("placed_in_service_on", period.period_end),
    service
      .from("books_fixed_asset_events")
      .select("asset_id,event_date")
      .eq("org_id", context.orgId)
      .eq("event_type", "depreciation")
      .gte("event_date", period.period_start)
      .lte("event_date", period.period_end),
  ]);
  ...
  const customerDepositsTieOut = tieOut("customer_deposits_control");
  const debtRegisterTieOut = tieOut("debt_register_control");
  const fixedAssetsTieOut = tieOut("fixed_assets_control");
  const depreciationTieOut = tieOut("accumulated_depreciation_control");
  const errors = [
    drafts.error,
    reconciliationItems.error,
    latestReconciliationRun.error,
    syncBacklog.error,
    waiverHolds.error,
    retainageMovement.error,
    journalProposals.error,
    unassignedTaxInvoices.error,
    unassignedUseTaxBills.error,
    fixedAssets.error,
    fixedAssetDepreciation.error,
    taxIdentities.error,
  ].filter(Boolean);
  if (errors.length > 0)
    throw new Error(
      `Failed to run Books close checklist: ${errors[0]?.message}`,
    );
  ...
  for (const row of bankReconciliations) {
    ...
    if (!current || row.statement_end > current)
      latestBankReconciliation.set(row.bank_account_id, row.statement_end);
  }
  ...
  for (const row of pocSnapshots) {
    ...
    if (!current || row.as_of > current)
      latestPoc.set(row.project_id, row.as_of);
  }
  ...
  const pocProjects = pocProjectBases.filter(
    (project) =>
      project.basis === "percentage_of_completion" &&
      (project.status === "active" || project.status === "on_hold"),
  );
  const missingPoc = pocProjects.filter(
    (project) => latestPoc.get(project.projectId) !== period.period_end,
  );
  ...
  const uncodedBills = openBills.filter(
    (bill) =>
      ...
      (typeof bill.accounting_coding === "object" &&
        Object.keys(bill.accounting_coding).length === 0),
  );
  const vaultedTaxCompanyIds = new Set(
    (taxIdentities.data ?? []).map((identity) => String(identity.company_id)),
  );
  const taxExceptions = taxVendors.filter(
    (vendor) =>
      !vendor.w9_file_id || !vaultedTaxCompanyIds.has(String(vendor.id)),
  );
  const unpushed = syncBacklog.data ?? [];
  const heldPayments = waiverHolds.data ?? [];
  const retainageRows = retainageMovement.data ?? [];
  const retainageMovedCents = retainageRows.reduce(
  ...
  const reconciliationStale =
    ...
    String(reconciliationRun.run_date) < period.period_end;
  const depreciatedAssetIds = new Set(
    (fixedAssetDepreciation.data ?? []).map((row) => row.asset_id),
  );
  const missingDepreciation = (fixedAssets.data ?? []).filter(
    (asset) => asset.status === "active" && !depreciatedAssetIds.has(asset.id),
  );
  ...
  const checks: CloseCheck[] = [
    {
      ...
      category: "ledger",
      blocking: true,
      status: drafts.count ? "failed" : "passed",
      issueCount: drafts.count ?? 0,
      href: "/books/ledger",
      evidence: {},
    },
    {
      code: "journal_proposals",
      label: "Manual journal proposals independently reviewed",
      category: "ledger",
      blocking: true,
      status: journalProposals.count ? "failed" : "passed",
      issueCount: journalProposals.count ?? 0,
      href: "/books/ledger",
      evidence: {},
    },
    {
      code: "bank_reconciliations",
      label: "Bank and card accounts reconciled",
      category: "cash",
      blocking: true,
      status: unreconciledAccounts.length ? "failed" : "passed",
      issueCount: unreconciledAccounts.length,
      href: "/books/banking",
      evidence: { account_ids: unreconciledAccounts.map((row) => row.id) },
    },
    {
      code: "bank_matches",
      label: "Bank transactions matched or excluded",
      category: "cash",
      blocking: true,
      status: unmatched.length ? "failed" : "passed",
      issueCount: unmatched.length,
      href: "/books/banking",
      evidence: { transaction_ids: unmatched.map((row) => row.id) },
    },
    {
      code: "accounting_drift",
      label: "Accounting reconciliation issues resolved",
      category: "integrations",
      blocking: true,
      status: reconciliationItems.count ? "failed" : "passed",
      issueCount: reconciliationItems.count ?? 0,
      href: "/books/close",
      evidence: {
        run_id: reconciliationRun?.id ?? null,
        run_date: reconciliationRun?.run_date ?? null,
        run_status: reconciliationRun?.status ?? null,
      },
    },
    {
      code: "reconciliation_freshness",
      label: "Reconciliation swept through period end",
      category: "integrations",
      blocking: false,
      status: reconciliationStale ? "warning" : "passed",
      issueCount: reconciliationStale ? 1 : 0,
      href: "/books/close",
      evidence: {
        run_date: reconciliationRun?.run_date ?? null,
        run_status: reconciliationRun?.status ?? null,
        scan_capped:
          reconciliationRun?.checked_counts &&
          typeof reconciliationRun.checked_counts === "object"
            ? (reconciliationRun.checked_counts as { scan_capped?: unknown })
                .scan_capped === true
            : false,
      },
    },
    {
      code: "poc_snapshots",
      label: "WIP and POC captured through period end",
      category: "construction",
      blocking: true,
      status: missingPoc.length ? "failed" : "passed",
      issueCount: missingPoc.length,
      href: "/reports/wip-over-under",
      evidence: { project_ids: missingPoc.map((row) => row.projectId) },
    },
    {
      ...
      category: "controls",
      blocking: true,
      status: jobCostTieOut.status,
      issueCount: jobCostTieOut.status === "failed" ? 1 : 0,
      evidence: {
        ledger_cents: jobCostTieOut.ledgerCents,
        subledger_cents: jobCostTieOut.subledgerCents,
        difference_cents: jobCostTieOut.differenceCents,
      },
    },
    {
      code: "ar_control",
      ...
      category: "controls",
      blocking: true,
      status: arTieOut.status,
      issueCount: arTieOut.status === "failed" ? 1 : 0,
      evidence: {
        ledger_cents: arTieOut.ledgerCents,
        subledger_cents: arTieOut.subledgerCents,
        difference_cents: arTieOut.differenceCents,
      },
    },
    {
      code: "ap_control",
      ...
      category: "controls",
      blocking: true,
      status: apTieOut.status,
      issueCount: apTieOut.status === "failed" ? 1 : 0,
      evidence: {
        ledger_cents: apTieOut.ledgerCents,
        subledger_cents: apTieOut.subledgerCents,
        difference_cents: apTieOut.differenceCents,
      },
    },
    {
      ...
      category: "controls",
      blocking: true,
      status: customerDepositsTieOut.status,
      issueCount: customerDepositsTieOut.status === "failed" ? 1 : 0,
      evidence: {
        ledger_cents: customerDepositsTieOut.ledgerCents,
        subledger_cents: customerDepositsTieOut.subledgerCents,
        difference_cents: customerDepositsTieOut.differenceCents,
      },
    },
    ...[
      {
      ...
      (item): CloseCheck => ({
        ...
        category: "controls",
        blocking: true,
        status: item.tieOut.status,
        issueCount: item.tieOut.status === "failed" ? 1 : 0,
        href: "/books",
        evidence: {
          ledger_cents: item.tieOut.ledgerCents,
          subledger_cents: item.tieOut.subledgerCents,
          difference_cents: item.tieOut.differenceCents,
        },
      }),
    ),
    {
      code: "clearing_accounts",
    ...
    },
    {
      ...
      label: "Fixed-asset depreciation posted for the period",
      category: "controls",
      blocking: true,
      status: missingDepreciation.length ? "failed" : "passed",
      issueCount: missingDepreciation.length,
      href: "/books",
      evidence: { asset_ids: missingDepreciation.map((asset) => asset.id) },
    },
    {
      code: "tax_jurisdictions",
      label: "Taxable sales and use-tax accruals have jurisdictions",
      category: "tax",
      blocking: true,
      status:
        (unassignedTaxInvoices.count ?? 0) +
          (unassignedUseTaxBills.count ?? 0) >
        0
          ? "failed"
          : "passed",
      issueCount:
        (unassignedTaxInvoices.count ?? 0) + (unassignedUseTaxBills.count ?? 0),
      href: "/books/accountant",
      evidence: {
        invoice_count: unassignedTaxInvoices.count ?? 0,
        bill_count: unassignedUseTaxBills.count ?? 0,
      },
    },
    {
      code: "coding_exceptions",
      label: "Approved bills are coded",
      category: "ledger",
      blocking: true,
      status: uncodedBills.length ? "failed" : "passed",
      issueCount: uncodedBills.length,
      href: "/payables?tab=approval",
      evidence: { bill_ids: uncodedBills.map((row) => row.id) },
    },
    {
      code: "sync_backlog",
      label: "Approved records pushed to the external system",
      category: "integrations",
      blocking: true,
      status: closeScanStatus({
        issueCount: unpushed.length,
        scanCapped: unpushed.length >= SYNC_BACKLOG_SCAN_LIMIT,
        failedStatus: "failed",
      }),
      issueCount: unpushed.length,
      href: "/settings/integrations",
      evidence: {
        entity_ids: unpushed.slice(0, 100).map((row) => row.entity_id),
        scan_capped: unpushed.length >= SYNC_BACKLOG_SCAN_LIMIT,
      },
    },
    {
      code: "waiver_holds",
      label: "Waiver-held payables reviewed",
      category: "controls",
      blocking: false,
      status: closeScanStatus({
        issueCount: heldPayments.length,
        scanCapped: heldPayments.length >= SYNC_BACKLOG_SCAN_LIMIT,
        failedStatus: "warning",
      }),
      issueCount: heldPayments.length,
      href: "/payables?tab=ready",
      evidence: {
        bill_ids: heldPayments.slice(0, 100).map((row) => row.id),
        scan_capped: heldPayments.length >= SYNC_BACKLOG_SCAN_LIMIT,
      },
    },
    {
      code: "retainage_movement",
      label: "Retainage released this period reviewed",
      category: "construction",
      blocking: false,
      status: closeScanStatus({
        issueCount: retainageRows.length,
        scanCapped: retainageRows.length >= SYNC_BACKLOG_SCAN_LIMIT,
        failedStatus: "warning",
      }),
      issueCount: retainageRows.length,
      href: "/reports/retainage",
      evidence: {
        released_cents: retainageMovedCents,
        retainage_ids: retainageRows.slice(0, 100).map((row) => row.id),
        scan_capped: retainageRows.length >= SYNC_BACKLOG_SCAN_LIMIT,
      },
    },
    {
      code: "tax_readiness",
      label: "W-9 and tax identity exceptions reviewed",
      category: "tax",
      blocking: false,
      status: taxExceptions.length ? "warning" : "passed",
      issueCount: taxExceptions.length,
      href: "/books/accountant",
      evidence: { company_ids: taxExceptions.map((row) => row.id) },
    },
  ];
  const { error: itemError } = await service.from("books_close_items").upsert(
    checks.map((check) => ({
      org_id: context.orgId,
      period_id: period.id,
      code: check.code,
      ...
      evidence: check.href
        ? { ...check.evidence, href: check.href }
        : check.evidence,
    })),
    { onConflict: "period_id,code" },
  );
  if (itemError)
    throw new Error(`Failed to persist close checklist: ${itemError.message}`);
  return {
    period,
    checks,
...
export async function closeAccountingPeriod(periodId: string, orgId?: string) {
  const context = await requirePeriodPermission("books.close", orgId);
  const service = createServiceSupabaseClient();
  const { data: priorStatusData, error: beginError } = await service.rpc(
    "begin_books_period_close",
    {
  ...
  if (beginError)
    throw new Error(
      `Failed to begin accounting-period close: ${beginError.message}`,
    );
  const priorStatus = z.enum(["open", "reopened"]).parse(priorStatusData);
  try {
    ...
    if (checklist.blockingFailures.length > 0) {
      throw new Error(
        `Close blocked: ${checklist.blockingFailures.map((item) => item.label).join(", ")}`,
      );
    }
    ...
    const recognition = await recognizeRevenueForPeriod(
      context.orgId,
      period.period_end,
    );
    ...
    const withoutPosition = recognition.results.filter(
      (row) =>
        !row.posted &&
        row.skippedReason !== "no_delta" &&
        checklist.pocProjectIds.includes(row.projectId),
    );
    ...
    if (trialBalance.totalDebitCents !== trialBalance.totalCreditCents)
      throw new Error("Trial balance does not balance");
    if (balanceSheet.differenceCents !== 0)
    ...
    const officialStatements = [
      ...
      cashFlow,
      generalLedger,
      { statement: "ar_aging" as const, ...arAging },
      { statement: "ap_aging" as const, ...apAging },
      { statement: "wip" as const, ...wip },
    ];
    ...
    const { error: snapshotError } = await service
      ...
      .upsert(
        ...
        {
          onConflict: "period_id,statement_type,basis,content_hash",
          ignoreDuplicates: true,
        },
      );
    if (snapshotError)
      throw new Error(
        `Failed to snapshot financial statements: ${snapshotError.message}`,
      );
    ...
    if (closeError)
      throw new Error(
        `Failed to close accounting period: ${closeError.message}`,
      );
    await Promise.all([
      recordEvent({
        orgId: context.orgId,
        actorId: context.userId,
        eventType: "books.period_closed",
        entityType: "accounting_period",
        entityId: period.id,
        payload: {
          period_end: period.period_end,
          digest,
          revenue_recognized_cents: recognition.postedCents,
          revenue_recognized_projects: recognition.recognized,
        },
      }),
      recordAudit({
        orgId: context.orgId,
    ...
    if (cancelError) {
      throw new Error(
        `${closeError instanceof Error ? closeError.message : String(closeError)}; additionally failed to release the close lock: ${cancelError.message}`,
      );
    }
    throw closeError;
  }
...
export async function closeFiscalYearToRetainedEarnings(
  periodId: string,
  orgId?: string,
) {
  const context = await requirePeriodPermission("books.close", orgId);
  ...
  const { data, error } = await service
    ...
    .single();
  if (error)
    throw new Error(`Failed to load year-end period: ${error.message}`);
  const period = periodSchema.parse(data);
  if (period.fiscal_period !== 12 && period.fiscal_period !== 13)
    throw new Error(
      "Retained-earnings close is only available for the final fiscal period",
    );
...
export async function reopenAccountingPeriod(input: {
  ...
  if (input.reason.trim().length < 10)
    throw new Error("A substantive reopen reason is required");
  const service = createServiceSupabaseClient();
  const { data: before, error: loadError } = await service
    .from("accounting_periods")
    .select("status, close_digest")
    .eq("org_id", context.orgId)
    .eq("id", input.periodId)
    .single();
  if (loadError || before?.status !== "closed")
    throw new Error("Only a closed accounting period can be reopened");
  const { error } = await service
    ...
    .eq("status", "closed");
  if (error)
    throw new Error(`Failed to reopen accounting period: ${error.message}`);
  await recordEvent({
    orgId: context.orgId,
    actorId: context.userId,
    eventType: "books.period_reopened",
    entityType: "accounting_period",
    entityId: input.periodId,
    payload: { reason: input.reason.trim(), prior_digest: before.close_digest },
  });
}
