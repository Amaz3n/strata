...
export async function refreshAccountingConnectionToken(connectionId: string, orgId?: string) {
  const { supabase, orgId: resolvedOrgId } = await requireOrgContext(orgId)
  const { data } = await supabase.from("accounting_connections").select("id,provider,label").eq("org_id", resolvedOrgId).eq("id", connectionId).maybeSingle()
...
export async function disconnectAccountingConnection(connectionId: string, orgId?: string) {
  ...
  if (isAccountingProviderKey(connection.provider)) {
    try {
      await getProvider(connection.provider).disconnect({ orgId: resolvedOrgId, connectionId })
    } catch (revokeError) {
  ...
  await recordEvent({ orgId: resolvedOrgId, actorId: userId, eventType: "accounting_disconnected", entityType: "accounting_connection", entityId: connectionId, payload: { provider: connection.provider, label: connection.label }, channel: "integration" })
}
