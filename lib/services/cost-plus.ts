...
import {
  learnCodingRule,
  suggestCoding,
} from "@/lib/services/books/coding-rules";
import {
...
function applyRetainageToInvoiceDraft(
  ...
  const retainageCents = Math.round(
    Math.max(retainageBaseCents, 0) * (effectivePercent / 100),
  );
...
function mapBillableCost(row: any): BillableCost {
  ...
  return {
    ...
    billable_cents:
      row.billable_cents ?? (row.cost_cents ?? 0) + (row.markup_cents ?? 0),
...
export async function listMarkupRules(orgId?: string): Promise<MarkupRule[]> {
  ...
  const { data, error } = await supabase
    .from("markup_rules")
    .select(
      "*, contract:contracts(id, title), cost_code:cost_codes(id, code, name)",
    )
...
export async function createMarkupRule(
  ...
  const payload = {
    ...
    effective_from: parsed.effectiveFrom
      ? toDateOnly(parsed.effectiveFrom)
      : null,
  ...
  if (error || !data)
    throw new Error(`Failed to create markup rule: ${error?.message}`);
  await recordAudit({
    orgId: resolvedOrgId,
    actorId: userId,
    action: "insert",
...
export async function getProjectCostContract(
  supabase: SupabaseClient,
  orgId: string,
  projectId: string,
) {
...
function isTimeAndMaterialsContract(contract: any | null) {
  return (
    contract?.contract_type === "time_materials" ||
    contract?.snapshot?.billing_model === "time_and_materials"
  );
}
...
async function ensureAllowanceOverageBillableCosts({
  ...
  for (const allowance of allowances ?? []) {
    const rawOverageCents =
      Number(allowance.used_cents ?? 0) - Number(allowance.budget_cents ?? 0);
    const overageCents =
      allowance.overage_handling === "absorb" ||
      allowance.overage_handling === "client_direct"
        ? 0
    ...
    if (existingError)
      throw new Error(
        `Failed to load allowance overage ledger rows: ${existingError.message}`,
      );
    const allowanceRows = (existingRows ?? []).filter(
      (row: any) =>
        row.source_id === allowance.id ||
        row.metadata?.allowance_id === allowance.id,
    );
    const postedOverageCents = allowanceRows.reduce(
      (sum: number, row: any) => sum + Number(row.cost_cents ?? 0),
      0,
    );
...
export async function resolveMarkupPercentsBatch(args: {
  ...
  const [costCodesResult, contractResult, rulesResult] = await Promise.all([
    ...
    args.supabase
      .from("markup_rules")
      .select(
        "scope, contract_id, cost_code_id, markup_percent, applies_to_category, effective_from, effective_to, created_at",
      )
      .eq("org_id", args.orgId)
      .order("created_at", { ascending: false }),
  ]);

  if (costCodesResult.error)
    throw new Error(
      `Failed to load cost code markups: ${costCodesResult.error.message}`,
    );
  if (contractResult.error)
    throw new Error(
      `Failed to load contract markup: ${contractResult.error.message}`,
    );
  if (rulesResult.error)
    throw new Error(
      `Failed to load markup rules: ${rulesResult.error.message}`,
    );

  const costCodeById = new Map<string, any>(
    (costCodesResult.data ?? []).map((costCode: any) => [
      costCode.id,
      costCode,
    ]),
  );
  ...
  return args.costs.map((cost) => {
    ...
    const costCodeCategory =
      cost.costCodeCategory ?? costCode?.category ?? null;
    ...
    if (args.contractId) {
      ...
      if (
        rawContractMarkup != null &&
        rawContractMarkup !== "" &&
        Number.isFinite(contractMarkup)
      ) {
    ...
    if (
      costCode?.default_markup_percent != null &&
      costCode.default_markup_percent !== "" &&
      Number.isFinite(defaultCostCodeMarkup)
    ) {
    ...
    const orgRule = rules.find(
      (rule: any) =>
        rule.scope === "org" && matchesRule(rule, costWithCategory),
    );
    if (orgRule)
...
async function insertOrReturnBillableCost(
  ...
  const duplicate =
    ...
    String(error?.message ?? "")
      ...
      .includes("duplicate");
  if (!duplicate)
    throw new Error(`Failed to create billable cost: ${error?.message}`);
  ...
  if (existingError || !existing)
    throw new Error(
      `Failed to load existing billable cost: ${existingError?.message}`,
    );
...
export async function upsertBillableCostFromBillLine(args: {
  ...
  const { supabase, orgId: resolvedOrgId } = await requireOrgContext(
    args.orgId,
  );
  ...
  const { data: line, error } = await supabase
    ...
    .select(
      ...
      bill:vendor_bills(id, org_id, project_id, bill_number, bill_date, status, use_tax_accrued_cents, commitment:commitments(company_id))
    `,
    )
    .eq("org_id", resolvedOrgId)
  ...
  if (!isCostPlusContract(contract))
    throw new Error("Project contract is not cost-plus or T&M");
...
export async function upsertBillableCostFromExpense(args: {
  ...
  if (expense.status !== "approved")
    throw new Error("Expense must be approved before it enters the ledger");
  ...
  const isBillable =
    ...
    (expense as any).cost_code?.is_reimbursable_default !== false;
  const gmpClassification = await resolveGmpClassificationForCostSource({
    supabase,
    orgId: resolvedOrgId,
    projectId: expense.project_id,
  ...
  const billable = await insertOrReturnBillableCost(
    ...
    resolvedOrgId,
    {
      ...
      description:
        expense.description || expense.vendor_name_text || "Project expense",
      cost_cents: costCents,
      markup_percent_resolved: markup.percent,
      markup_cents: calculateMarkupCents(costCents, markup.percent),
      is_billable: isBillable,
  ...
  await supabase
    .from("project_expenses")
    .update({ billable_cost_id: billable.id })
    .eq("org_id", resolvedOrgId)
    .eq("id", expense.id);
  return billable;
}

export async function upsertBillableCostFromExpenseLine(args: {
...
export async function upsertBillableCostFromTimeEntry(args: {
  ...
  const markup = tmRate
    ? {
        percent: derivedMarkupPercent(
          costCents,
          tmRate.billableCents - costCents,
        ),
        source:
  ...
  const isBillable =
    ...
    (entry as any).cost_code?.is_reimbursable_default !== false;
  const markupCents = tmRate
    ? tmRate.billableCents - costCents
    : calculateMarkupCents(costCents, markup.percent);
...
export async function propagateApprovalToLedger(args: {
  ...
  const scanVariance = async (projectIds: Array<string | null | undefined>) => {
    for (const projectId of Array.from(
  ...
  if (args.source === "project_expense") {
    ...
    if (linesError)
      throw new Error(`Failed to load expense splits: ${linesError.message}`);
    ...
    if ((lines ?? []).length > 0) {
      ...
      const projectIds = Array.from(
        new Set(lines!.map((line) => line.project_id ?? expense.project_id)),
      );
      ...
      await Promise.all(
        lines!
          .filter((line) =>
            contractByProject.get(line.project_id ?? expense.project_id),
          )
          .map((line) =>
            upsertBillableCostFromExpenseLine({
    ...
    await postJobCostEntriesForProjectExpense({
      expenseId: args.sourceId,
      orgId: resolvedOrgId,
    });
    await scanVariance(
      (lines ?? []).length > 0
        ? lines!.map((line) => line.project_id ?? expense.project_id)
        : [expense.project_id],
    );
    return;
  }
...
export async function createTimeEntry(input: TimeEntryInput, orgId?: string) {
  ...
  let burdenMultiplier =
    parsed.burdenMultiplier ?? Number(contract?.labor_burden_multiplier ?? 1);
  ...
  if (workerUserId) {
    const { data: laborDefaults } = await supabase
      .from("memberships")
      .select(
        "labor_cost_rate_cents, labor_burden_multiplier, labor_is_billable_default",
      )
      .eq("org_id", resolvedOrgId)
    ...
    if (laborDefaults) {
      if (!baseRateCents)
        baseRateCents = laborDefaults.labor_cost_rate_cents ?? 0;
      if (!parsed.burdenMultiplier || parsed.burdenMultiplier === 1) {
        burdenMultiplier = Number(
          laborDefaults.labor_burden_multiplier ?? burdenMultiplier,
        );
      }
...
export async function updateTimeEntry(
  ...
  if (parsed.costCodeId !== undefined)
    updates.cost_code_id = parsed.costCodeId ?? null;
  if (parsed.baseRateCents !== undefined)
    updates.base_rate_cents = parsed.baseRateCents;
  if (parsed.burdenMultiplier !== undefined)
    updates.burden_multiplier = parsed.burdenMultiplier;
  ...
  if (parsed.otMultiplier !== undefined)
    updates.ot_multiplier = parsed.otMultiplier;
  if (parsed.isDoubleTime !== undefined)
    updates.is_double_time = parsed.isDoubleTime;
  if (parsed.dtMultiplier !== undefined)
    updates.dt_multiplier = parsed.dtMultiplier;
  ...
  if (impactsPostedLedger) {
    ...
    if (costError)
      throw new Error(
        `Failed to load time entry ledger rows: ${costError.message}`,
      );
    if (
      (existingCosts ?? []).some(
        (cost: any) => cost.invoice_id || cost.status === "billed",
      )
    ) {
      throw new Error(
        "This time entry has costs already billed on an invoice. Remove them from the invoice before changing the time entry.",
      );
    }
  }
...
export async function rejectTimeEntry(
  ...
  if (
    ["pm_approved", "client_approved", "locked"].includes(String(before.status))
  ) {
    await voidJobCostEntryForSource({
      sourceType: "time_entry",
      sourceId: data.id,
...
export async function createTimeEntryApprovalLink(
  timeEntryId: string,
  orgId?: string,
) {
  const {
  ...
  if (!["submitted", "pm_approved"].includes(entry.status)) {
    throw new Error(
      "Only submitted or PM-approved time entries can be sent for client approval",
    );
  }
  ...
  if (updateError)
    throw new Error(`Failed to create approval link: ${updateError.message}`);
  ...
  return {
    ...
    expiresAt,
    url: appUrl
      ? `${appUrl}/api/time-entries/approve/${token}`
      : `/api/time-entries/approve/${token}`,
  };
}

export async function sendTimeEntryClientApprovalEmail(
  timeEntryId: string,
  orgId?: string,
) {
  const {
  ...
  const [{ data: entry, error }, { data: org }] = await Promise.all([
    supabase
      .from("time_entries")
      .select(
        "id, project_id, worker_name, work_date, hours, notes, cost_cents, project:projects(name)",
      )
      .eq("org_id", resolvedOrgId)
      .eq("id", timeEntryId)
      .maybeSingle(),
  ...
  const approval = await createTimeEntryApprovalLink(
    timeEntryId,
    resolvedOrgId,
  );
  ...
  const contact = Array.isArray((tokenRow as any)?.contact)
    ? (tokenRow as any).contact[0]
    : (tokenRow as any)?.contact;
  const recipientEmail = contact?.email;
  if (!recipientEmail) {
    throw new Error("No client portal contact email found for this project");
  }

  const projectName = Array.isArray((entry as any).project)
    ? (entry as any).project[0]?.name
...
export async function approveTimeEntryByToken(token: string) {
  ...
  if (
    entry.approval_token_expires_at &&
    new Date(entry.approval_token_expires_at) < new Date()
  ) {
    throw new Error("Approval link has expired");
  }
  if (entry.status === "client_approved") return entry;
  if (entry.status !== "pm_approved")
    throw new Error("Time entry is not ready for client approval");
...
export async function replaceProjectExpenseLines(args: {
  ...
  const {
    ...
    orgId: resolvedOrgId,
    userId,
  } = await requireOrgContext(args.orgId);
  ...
  if (alreadyPosted) {
      ...
      .or(
        [
          ...
          priorLineIds.length > 0
            ? `and(source_type.eq.project_expense_line,source_id.in.(${priorLineIds.join(",")}))`
            : null,
        ]
          .filter(Boolean)
          .join(","),
    ...
    if ((billed.data ?? []).length > 0) {
      throw new Error(
        "This expense has costs already billed on an invoice. Remove them from the invoice before changing the split.",
      );
    }
  }
  ...
  if (lines.length > 0) {
    ...
    const allocated = lines.reduce(
      (sum, line) => sum + Math.round(Number(line.amount_cents ?? 0)),
      0,
    );
    if (allocated !== totalCents) {
      throw new Error(
        `Splits must total ${(totalCents / 100).toFixed(2)} (currently ${(allocated / 100).toFixed(2)})`,
      );
    }
  ...
  if (lines.length > 0) {
    ...
    if (insertError)
      throw new Error(
        `Failed to update expense splits: ${insertError.message}`,
      );
...
async function resyncApprovedExpenseLedger(
  ...
  if (args.hasNewLines) {
    ...
    args.canPostBillable &&
    (await isCostPlusProject(args.expenseProjectId))
  ) {
...
export async function createProjectExpense(
  ...
  const hasExplicitAccountingCoding = Boolean(
    parsed.qboExpenseAccountId ||
    parsed.qboPaymentAccountId ||
    parsed.qboApAccountId,
  );
  const shouldApplySuggestion = Boolean(
    codingSuggestion?.autoApply &&
    !parsed.costCodeId &&
    !hasExplicitAccountingCoding,
  );
  ...
  if (!parsed.allowDuplicate) {
    ...
    if (duplicateError)
      throw new Error(
        `Failed to check for duplicate expenses: ${duplicateError.message}`,
      );

    const normalizedVendor =
      parsed.vendorNameText?.trim().toLowerCase() || null;
    const duplicate = (candidates ?? []).find((row) => {
      const existingVendor =
        (row.vendor_name_text as string | null)?.trim().toLowerCase() || null;
      return (
        !normalizedVendor ||
        !existingVendor ||
        existingVendor === normalizedVendor
      );
    });
    if (duplicate) {
      throw new Error(
        `${DUPLICATE_EXPENSE_ERROR_PREFIX} An expense for ${duplicate.vendor_name_text ?? "this vendor"} on this date with the same amount already exists.`,
      );
    }
  }
  ...
  const payload = {
    ...
    cost_code_id:
      parsed.costCodeId ??
      (shouldApplySuggestion ? codingSuggestion?.costCodeId : null) ??
      null,
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error || !data)
    throw new Error(`Failed to create project expense: ${error?.message}`);
  await recordAudit({
    orgId: resolvedOrgId,
    actorId: userId,
    action: "insert",
...
export async function approveProjectExpense(expenseId: string, orgId?: string) {
  ...
  assertApprovalAllowed(
    getExpenseApprovalBlockingReasons(before, gateSettings),
  );
...
export async function rejectProjectExpense(
  expenseId: string,
  input: { rejectionReason?: string | null } = {},
  orgId?: string,
) {
...
export async function createTimeEntryFromPortal({
  token,
  input,
}: {
  token: string;
...
export async function createProjectExpenseFromPortal({
  token,
  input,
}: {
  token: string;
...
export async function listProjectTimeEntries(
  ...
  if (reviewableTimeEntries.error)
    throw new Error(
      `Failed to load reviewable time entries: ${reviewableTimeEntries.error.message}`,
    );
  if (recentTimeEntries.error)
    throw new Error(
      `Failed to load recent time entries: ${recentTimeEntries.error.message}`,
    );

  return Array.from(
    new Map(
      [
        ...(reviewableTimeEntries.data ?? []),
        ...(recentTimeEntries.data ?? []),
...
export async function listCostPlusTabData(projectId: string, orgId?: string) {
  ...
  const [
    ...
    supabase
      .from("project_expenses")
      .select(
        "*, cost_code:cost_codes(code, name), vendor_company:companies(name)",
      )
      .eq("org_id", resolvedOrgId)
      .eq("project_id", projectId)
  ...
  if (billableCosts.error)
    throw new Error(
      `Failed to load billable costs: ${billableCosts.error.message}`,
    );
  if (reviewableTimeEntries.error)
    throw new Error(
      `Failed to load reviewable time entries: ${reviewableTimeEntries.error.message}`,
  ...
  if (reviewableExpenses.error)
    throw new Error(
      `Failed to load reviewable expenses: ${reviewableExpenses.error.message}`,
    );
  if (recentExpenses.error)
    throw new Error(
      `Failed to load recent expenses: ${recentExpenses.error.message}`,
    );
  ...
  const expenseRows = Array.from(
    new Map(
      [...(reviewableExpenses.data ?? []), ...(recentExpenses.data ?? [])].map(
        (expense: any) => [expense.id, expense],
      ),
    ).values(),
  );
  ...
  if (expenseIds.length > 0) {
    const { data: lineRows, error: lineError } = await supabase
      ...
      .order("sort_order", { ascending: true });
    if (lineError)
      throw new Error(`Failed to load expense lines: ${lineError.message}`);
...
export async function listOpenBookCostDetailsForInvoice({
  ...
  const { data, error } = await supabase
    .from("billable_costs")
    .select(
      "*, cost_code:cost_codes(code, name), source_company:companies(name)",
    )
    .eq("org_id", orgId)
    .eq("project_id", projectId)
  ...
  if (error)
    throw new Error(`Failed to load invoice cost detail: ${error.message}`);

  const timeEntryIds = (data ?? [])
    .filter((row: any) => row.source_type === "time_entry")
    .map((row: any) => row.source_id);
  const expenseIds = (data ?? [])
    .filter((row: any) => row.source_type === "project_expense")
    .map((row: any) => row.source_id);
  const billLineIds = (data ?? [])
    .filter((row: any) => row.source_type === "vendor_bill_line")
    .map((row: any) => row.source_id);
  ...
  const [timeEntries, expenses, billLines] = await Promise.all([
    timeEntryIds.length
      ? supabase
          .from("time_entries")
          .select("id, status, attached_file_ids")
          .eq("org_id", orgId)
          .in("id", timeEntryIds)
      : Promise.resolve({ data: [], error: null }),
    expenseIds.length
  ...
  if (timeEntries.error)
    throw new Error(`Failed to load time proof: ${timeEntries.error.message}`);
  if (expenses.error)
    throw new Error(`Failed to load expense proof: ${expenses.error.message}`);
  if (billLines.error)
    throw new Error(`Failed to load bill proof: ${billLines.error.message}`);

  const sourceProofById = new Map<
    string,
    { status?: string | null; proof_file_id?: string | null }
  >();
  for (const row of timeEntries.data ?? []) {
    sourceProofById.set(`time_entry:${row.id}`, {
      status: row.status ?? null,
      proof_file_id: Array.isArray(row.attached_file_ids)
        ? (row.attached_file_ids[0] ?? null)
        : null,
    });
  }
  ...
  for (const row of billLines.data ?? []) {
    const bill = Array.isArray((row as any).bill)
      ? (row as any).bill[0]
      : (row as any).bill;
  ...
  return (data ?? []).map((row: any) => ({
    ...
    source_status:
      sourceProofById.get(`${row.source_type}:${row.source_id}`)?.status ??
      null,
    proof_file_id:
      sourceProofById.get(`${row.source_type}:${row.source_id}`)
        ?.proof_file_id ??
      row.metadata?.receipt_file_id ??
      null,
  }));
}

export async function voidBillableCostsForVendorBill({
  billId,
  orgId,
}: {
  billId: string;
  ...
  if (error)
    throw new Error(`Failed to load billable costs for bill: ${error.message}`);
  for (const cost of costs ?? []) {
    if (cost.status === "locked") {
      throw new Error(
        "Vendor bill costs are currently locked by invoice creation. Refresh and try again.",
      );
    }
    if (cost.status === "billed") {
      const creditPayload = {
        ...
        is_billable: true,
        gmp_classification:
          cost.gmp_classification ??
          cost.metadata?.gmp_classification ??
          "inside_gmp",
        status: "open",
        metadata: {
          gmp_classification:
      ...
      const { error: voidError } = await supabase
        ...
        .eq("id", cost.id);
      if (voidError)
        throw new Error(`Failed to void billable cost: ${voidError.message}`);
    }
  }
}
...
export async function createManualBillableAdjustment(
  ...
  if (!isCostPlusContract(contract))
    throw new Error(
      "Manual billable adjustments are only available on cost-driven projects.",
    );

  const occurredOn = parsed.occurredOn
    ? toDateOnly(parsed.occurredOn)
    : toDateOnly(new Date());
  ...
  const created = await insertOrReturnBillableCost(
    supabase,
    resolvedOrgId,
    payload,
    "manual_adjustment",
...
export function buildInvoiceDraft({
  ...
  for (const cost of costs) {
    const key =
      groupBy === "detail" ? cost.id : (cost.cost_code_id ?? "uncoded");
    const feeKey =
      feePresentation === "separate_by_code"
        ? (cost.cost_code_id ?? "uncoded")
        : "builder_fee";
    ...
    const current =
      ...
      ({
        ...
        description:
          groupBy === "detail"
            ? cost.description || fallbackDescription
            : fallbackDescription,
        cost_cents: 0,
        markup_cents: 0,
        billable_cents: 0,
        markup_percent: separatesFee
          ? 0
          : Number(cost.markup_percent_resolved ?? 0),
        billable_cost_ids: [],
        metadata: {
    ...
    current.billable_cents += separatesFee
      ? cost.cost_cents
      : cost.billable_cents;
    ...
    if (separatesFee && cost.markup_cents !== 0) {
      ...
      const feeLine =
        ...
        ({
          cost_code_id:
            feePresentation === "separate_by_code"
              ? (cost.cost_code_id ?? null)
              : null,
          cost_code_code:
            feePresentation === "separate_by_code"
              ? (cost.cost_code_code ?? null)
              : null,
          cost_code_name:
            feePresentation === "separate_by_code"
              ? (cost.cost_code_name ?? null)
              : null,
      ...
      const relatedIds = (feeLine.metadata?.related_billable_cost_ids ??
        []) as string[];
  ...
  const costLines = Array.from(lineMap.values()).sort((a, b) =>
    `${a.cost_code_code ?? ""}${a.description}`.localeCompare(
      `${b.cost_code_code ?? ""}${b.description}`,
    ),
  );
...
function appendEarnedFeeLineToDraft(
  draft: InvoiceDraft,
  amountCents: number,
): InvoiceDraft {
...
export async function generateInvoiceFromCosts(
  ...
  const billingPeriod = parsed.billingPeriodId
    ...
    : null;
  if (parsed.billingPeriodId && !billingPeriod)
  ...
  if (to > today)
    throw new Error("Cannot generate an invoice for a future date range");
  if (from > to)
    throw new Error("Date range start must be before the end date");
  ...
  const contractProject = Array.isArray(contract.projects)
    ? contract.projects[0]
    : contract.projects;
      ...
      return {
        ...
        costCount:
          preview?.lines?.reduce(
            (sum: number, line: any) =>
              sum + (line.billable_cost_ids?.length ?? 0),
            0,
          ) ?? 0,
  ...
  if (parsed.costCodeIds?.length)
    query = query.in("cost_code_id", parsed.costCodeIds);
  ...
  for (const rawCost of rawCosts ?? []) {
    ...
    const markupCents = calculateMarkupCents(
      rawCost.cost_cents ?? 0,
      markup.percent,
    );
    ...
    if (
      ...
      if (refreshError)
        throw new Error(
          `Failed to refresh billable cost markup: ${refreshError.message}`,
        );
    }
  ...
  if (parsed.billableCostIds?.length) {
    ...
    excludedCountQuery = excludedCountQuery
  ...
  if (parsed.includeEarnedFee) {
    if (billingModel !== "cost_plus_fixed_fee") {
      throw new Error(
        "Earned fee can only be included for cost-plus fixed-fee projects.",
      );
    }
    if (parsed.dryRun) {
      const feeSummary = await getProjectFeeBillingSummary(
        parsed.projectId,
        resolvedOrgId,
      );
      if (!feeSummary.enabled)
        throw new Error(
          feeSummary.reason ?? "Fee billing is not available for this project.",
        );
  ...
  const preview = applyRetainageToInvoiceDraft(
    earnedFeeCents > 0
      ? appendEarnedFeeLineToDraft(costDraft, earnedFeeCents)
      : costDraft,
    contract.retainage_percent,
    Boolean(
      contract.retainage_applies_to_fee ??
      contract.snapshot?.retainage_applies_to_fee ??
      false,
    ),
  );
  ...
  if (billingModel === "cost_plus_gmp") {
    ...
    if (billedInsideGmp.error)
      throw new Error(
        `Failed to load billed GMP costs: ${billedInsideGmp.error.message}`,
      );
    ...
    const cumulativeInsideGmpCents =
      alreadyBilledInsideGmpCents + invoiceInsideGmpCents;
    const overageCents =
      cumulativeInsideGmpCents - Number(gmpSummary.revised_gmp_cents ?? 0);
    ...
    if (gmpSummary.enabled && overageCents > 0) {
      ...
      if (!parsed.dryRun && !parsed.overrideGmpCap) {
        throw new Error(
          `${warning.message} Check "bill anyway" to override the GMP cap.`,
        );
      }
  ...
  const approvedCostInvoice = await createApprovedCostInvoiceFromPreview({
    ...
    metadata: {
      ...
      retainage_applies_to_fee: Boolean(
        contract.retainage_applies_to_fee ??
        contract.snapshot?.retainage_applies_to_fee ??
        false,
      ),
      retainage_amount_cents: preview.totals.retainage_cents ?? null,
      gross_billable_cents:
        preview.totals.gross_billable_cents ?? preview.totals.billable_cents,
  ...
  if (preparedFeeBilling) {
    ...
    if (feeLineError)
      throw new Error(
        `Failed to load fee invoice line: ${feeLineError.message}`,
      );
    if (!feeLine?.id)
      throw new Error(
        "Approved-cost invoice was created without the earned fee line.",
      );
