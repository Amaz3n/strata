...
/**
 * The org's estimate accent color is user-supplied brand data, so these hex
 * strings are example/fallback VALUES for the color picker — not app styling.
 * The tokens-only rule targets styling literals; a color input has to speak hex.
 */
/* eslint-disable no-restricted-syntax -- brand-color picker data, see above */
const ACCENT_EXAMPLE = "#2563eb"
const ACCENT_PICKER_FALLBACK = "#1a1a1a"
...
function AppearanceDialog({
  ...
  const submit = async () => {
    ...
    if (trimmedColor && !HEX_PATTERN.test(trimmedColor)) {
      setError(`Use a 6-digit hex color like ${ACCENT_EXAMPLE}.`)
      return
    }
    setSaving(true)
    setError(null)
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2">
              <input
                ...
                value={HEX_PATTERN.test(color) ? color : ACCENT_PICKER_FALLBACK}
                onChange={(event) => {
