...
export function EstimateTemplatesClient({
  initialTemplates,
  costCodes,
}: Props) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<EstimateTemplateDto | null>(
    null,
  );

  const total = lines.reduce(
    (sum, l) =>
      l.item_type === "group" || l.is_optional
        ? sum
        : sum + (Number(l.unit_cost) || 0) * (Number(l.quantity) || 1),
    0,
  );
      ...
      try {
        if (editingId === "new") {
          const created = unwrapAction(
            await createEstimateTemplateAction(payload),
          );
          setTemplates((prev) =>
          ...
          const updated = unwrapAction(
            await updateEstimateTemplateAction(editingId, payload),
          );
          setTemplates((prev) =>
            prev
      ...
      {editingId ? (
        <PresetTemplateWorkspace
          ...
          rows={lines
            ...
            .map((l) => ({
              label: l.description,
              heading: l.item_type === "group",
              detail:
                l.item_type === "group"
                  ? undefined
                  : `${l.quantity || 1} × ${money(Math.round((Number(l.unit_cost) || 0) * 100))}${l.is_optional ? " · Optional" : ""}`,
              value:
                l.item_type === "group"
                  ? undefined
                  : money(
          ...
          total={`Base total ${money(Math.round(total * 100))}`}
          onClose={cancel}
          onSave={save}
          busy={pending}
        >
            ...
            <div className="space-y-2.5">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    onClick={() => setLines((p) => [...p, NEW_SECTION()])}
                  >
              ...
              {lines.map((line, idx) =>
                line.item_type === "group" ? (
                  <div
                    key={idx}
                    className="flex items-center gap-2 rounded-md border border-dashed bg-muted/30 p-2.5"
                  >
                  ...
                  <div key={idx} className="space-y-2 border bg-muted/20 p-3">
                      ...
                      <div className="w-28 space-y-1">
                        ...
                        <div className="relative">
                          ...
                          <Input
                            ...
                            placeholder="0.00"
                            className={cn("h-9 pl-7 tabular-nums", noSpinner)}
                          />
                        </div>
                      </div>
                      <Button
                    ...
                    <div className="flex items-center justify-between gap-2">
                      <Select
                        ...
                        onValueChange={(v) =>
                          updateLine(idx, {
                            cost_code_id: v === "none" ? undefined : v,
                          })
                        }
                      >
                      ...
                      <label className="flex items-center gap-2 text-xs text-muted-foreground">
                        <Checkbox
                          ...
                          onCheckedChange={(c) =>
                            updateLine(idx, { is_optional: c === true })
                          }
                        />
                        Optional add-on
                      </label>
          ...
          <Card>
            <CardContent className="flex flex-col items-center justify-center gap-2 py-10 text-center">
              ...
              <p className="max-w-sm text-xs text-muted-foreground">
                Create a template to start estimates faster — pre-built
                sections, line items, and optional add-ons.
              </p>
            </CardContent>
          </Card>
        ) : (
          templates.map((template) => {
            const lineCount = template.lines.filter(
              (l) => l.item_type !== "group",
            ).length;
            const sectionCount = template.lines.filter(
              (l) => l.item_type === "group",
            ).length;
            return (
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              “{deleteTarget?.name}” will be removed. Estimates already created
