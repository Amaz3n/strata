...
import { LayoutDashboard, FileText, HelpCircle, ShieldCheck, CheckSquare, ClipboardList } from "lucide-react"
import { cn } from "@/lib/utils"

export type SubPortalTab = "dashboard" | "documents" | "daily-logs" | "rfis" | "submittals" | "punch" | "compliance" | "prequalification"
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-20 border-t bg-background pb-[env(safe-area-inset-bottom)]">
      <div className="flex h-14">
        {tabs.filter((tab) => (tab.id !== "punch" || showPunch) && (tab.id !== "daily-logs" || showDailyLogs)).map((tab) => {
          const Icon = tab.icon
