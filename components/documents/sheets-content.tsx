...
export function SheetsContent({
  ...
  const [setRevisionTarget, setSetRevisionTarget] = useState<DrawingSet | null>(
    null,
  );
  ...
  const [deletingSheet, setDeletingSheet] = useState<{
    sheet: DrawingSheet;
  } | null>(null);
  ...
  const totalPages = Math.max(
    1,
    Math.ceil(filteredSheets.length / SHEETS_PAGE_SIZE),
  );
  ...
  return (
    <>
              ...
              <div className="min-w-0">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>
                    {
                      DRAWING_SET_TYPE_LABELS[
                        coerceSetType(selectedSet.set_type)
                      ]
                    }
                  </span>
                  <span aria-hidden>•</span>
                  <span>
                    Uploaded {formatCompactDate(selectedSet.created_at)}
                  </span>
                  <span aria-hidden>•</span>
                  <span>
                    Updated {formatCompactDate(selectedSet.updated_at)}
                  </span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-1">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end" className="w-52">
                    ...
                    <DropdownMenuItem
                      onClick={() => openSetRevisionDialog(selectedSet)}
                    >
            ...
            {selectedSet.status === "processing" && (
              <div className="mt-3 max-w-md">
                <div className="mb-1 text-[11px] text-muted-foreground">
                  Processing {selectedSet.processed_pages}/
                  {selectedSet.total_pages ?? "?"} pages
                </div>
                <Progress
                  value={
                    selectedSet.total_pages
                      ? (selectedSet.processed_pages /
                          selectedSet.total_pages) *
                        100
                      : 0
                  }
        ...
        {selectedSet.status === "ready" && (
          <>
            ...
            {!loadingSheets && filteredSheets.length === 0 && (
              <div className="flex items-center justify-center border bg-card px-6 py-16 text-sm text-muted-foreground">
                {searchQuery
                  ? "No sheets match the current search."
                  : "No sheets in this drawing set yet."}
              </div>
            )}
            ...
            {!loadingSheets && filteredSheets.length > 0 && (
              <>
                <SheetsTable
                  ...
                  onSheetClick={(sheet) =>
                    onSheetClick?.(sheet, filteredSheets)
                  }
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setSheetPage(Math.min(totalPages, clampedPage + 1))
                      }
                    >
                      Next
                    </Button>
          ...
          <div className="space-y-3 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <select
                ...
                value={setType}
                onChange={(event) =>
                  setSetType(coerceSetType(event.target.value))
                }
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload a new PDF for this set revision.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                disabled={uploadingSetRevision}
                onChange={(event) =>
                  setSetRevisionFile(event.target.files?.[0] ?? null)
                }
              />
            </div>
          </div>
      ...
      >
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Upload a replacement file and create a new revision.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-3 py-2">
            <div className="rounded-md border bg-muted/30 px-3 py-2 text-sm">
              <p className="font-medium">
                {versionTarget?.sheet.sheet_number}{" "}
                {versionTarget?.sheet.sheet_title ?? ""}
              </p>
              <p className="text-xs text-muted-foreground">
          ...
          <DialogFooter>
            ...
            <Button
              onClick={submitSheetVersion}
              disabled={savingVersion || loadingVersionMeta || !versionFile}
            >
              {savingVersion ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently removes &ldquo;{deletingSet?.title}&rdquo; and
              all linked sheets and versions.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deletingSetPending}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
      ...
      <AlertDialog
        ...
        onOpenChange={(open) => !open && setDeletingSheet(null)}
      >
        <AlertDialogContent>
          <AlertDialogHeader>
          ...
          <AlertDialogFooter>
            <AlertDialogCancel disabled={deletingSheetPending}>
              Cancel
            </AlertDialogCancel>
            <AlertDialogAction
