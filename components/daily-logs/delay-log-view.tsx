...
export function DelayLogView({ reports, scheduleItems }: { reports: DailyReport[]; scheduleItems: ScheduleItem[] }) {
  ...
  return <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
    ...
    <div className="min-h-0 flex-1 overflow-auto">
      {delays.length === 0 ? <div className="grid min-h-64 place-items-center text-center"><div><AlertTriangle className="mx-auto h-6 w-6 text-muted-foreground" /><p className="mt-2 text-sm font-medium">No matching delays</p><p className="text-xs text-muted-foreground">Logged delays appear here with their schedule linkage.</p></div></div> : <table className="w-full text-sm"><thead className="sticky top-0 bg-background text-left text-[10px] uppercase tracking-wider text-muted-foreground"><tr className="border-b"><th className="px-6 py-2 font-medium">Date / time</th><th className="px-3 py-2 font-medium">Type</th><th className="px-3 py-2 font-medium">Description</th><th className="px-3 py-2 font-medium">Schedule item</th><th className="px-3 py-2 text-right font-medium">Hours</th><th className="px-3 py-2 font-medium">Owner notice</th><th className="px-6 py-2 font-medium">Claim</th></tr></thead><tbody>{delays.map((delay) => <tr key={delay.id} className="border-b border-border/60"><td className="whitespace-nowrap px-6 py-3 font-mono text-xs tabular-nums"><span className="inline-flex items-center gap-1.5"><CalendarDays className="h-3.5 w-3.5 text-muted-foreground" />{delay.reportDate}</span>{delay.delay_start_time && <div className="mt-0.5 text-muted-foreground">{delay.delay_start_time}{delay.delay_end_time ? `–${delay.delay_end_time}` : ""}</div>}</td><td className="px-3 py-3 capitalize">{delay.delay_type}</td><td className="max-w-md px-3 py-3"><p className="font-medium">{delay.description}</p>{delay.affected_trades && <p className="mt-0.5 text-xs text-muted-foreground">{delay.affected_trades}</p>}</td><td className="px-3 py-3 text-muted-foreground">{delay.schedule_item_id ? scheduleById.get(delay.schedule_item_id)?.name ?? "Removed item" : "—"}</td><td className="px-3 py-3 text-right font-mono text-xs tabular-nums">{delay.hours_lost ?? "—"}</td><td className="px-3 py-3 text-xs">{delay.owner_notice_sent ? <><Badge variant="outline">Sent {delay.owner_notice_date ?? ""}</Badge>{delay.owner_notice_reference && <div className="mt-1 max-w-44 truncate text-muted-foreground" title={delay.owner_notice_reference}>{delay.owner_notice_reference}</div>}</> : "—"}</td><td className="px-6 py-3">{delay.potential_claim ? <Badge variant="outline">Potential claim</Badge> : "—"}</td></tr>)}</tbody></table>}
    </div>
  </div>
}
