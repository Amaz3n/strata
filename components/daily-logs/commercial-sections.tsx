...
function rowMeta(kind: DailyReportSectionKind, row: Record<string, unknown>, scheduleItems: ScheduleItem[]) {
  if (kind === "delay") {
    ...
    const window = row.delay_start_time ? `${row.delay_start_time}${row.delay_end_time ? `–${row.delay_end_time}` : ""}` : null
    const notice = row.owner_notice_sent ? `Owner notice ${row.owner_notice_date ?? "sent"}${row.owner_notice_reference ? ` · ${row.owner_notice_reference}` : ""}` : null
    return [String(row.delay_type ?? "delay").replaceAll("_", " "), window, row.hours_lost != null ? `${row.hours_lost}h lost` : null, schedule?.name, notice, row.potential_claim ? "Potential claim" : null].filter(Boolean).join(" · ")
  }
...
function initialForm(kind: DailyReportSectionKind, row?: Record<string, unknown>) {
  if (kind === "delay") return { delay_type: row?.delay_type ?? "weather", description: row?.description ?? "", hours_lost: row?.hours_lost ?? "", affected_trades: row?.affected_trades ?? "", schedule_item_id: row?.schedule_item_id ?? "none", potential_claim: row?.potential_claim ?? false, delay_start_time: row?.delay_start_time ?? "", delay_end_time: row?.delay_end_time ?? "", owner_notice_sent: row?.owner_notice_sent ?? false, owner_notice_date: row?.owner_notice_date ?? "", owner_notice_reference: row?.owner_notice_reference ?? "" }
...
function SectionForm({ kind, initial, scheduleItems, busy, onCancel, onSubmit }: {
  ...
  function submit() {
    if (kind === "delay") onSubmit({ delay_type: form.delay_type as DelayInput["delay_type"], description: text("description"), hours_lost: numeric("hours_lost"), affected_trades: text("affected_trades") || undefined, schedule_item_id: text("schedule_item_id") === "none" ? null : text("schedule_item_id"), potential_claim: Boolean(form.potential_claim), delay_start_time: text("delay_start_time") || undefined, delay_end_time: text("delay_end_time") || undefined, owner_notice_sent: Boolean(form.owner_notice_sent), owner_notice_date: text("owner_notice_date") || undefined, owner_notice_reference: text("owner_notice_reference") || undefined })
  ...
  return (
    <div className="space-y-3 border border-border bg-muted/20 p-3">
      {kind === "delay" && <>
        ...
        <div className="grid grid-cols-2 gap-3"><div className="space-y-1"><Label>Delay start</Label><Input type="time" value={text("delay_start_time")} onChange={(event) => set("delay_start_time", event.target.value)} /></div><div className="space-y-1"><Label>Delay end</Label><Input type="time" value={text("delay_end_time")} onChange={(event) => set("delay_end_time", event.target.value)} /></div></div>
        ...
        <label className="flex items-center gap-2 text-sm"><Checkbox checked={Boolean(form.owner_notice_sent)} onCheckedChange={(value) => set("owner_notice_sent", value === true)} />Owner notice sent</label>
        {Boolean(form.owner_notice_sent) && <div className="grid grid-cols-2 gap-3"><div className="space-y-1"><Label>Notice date</Label><Input type="date" value={text("owner_notice_date")} onChange={(event) => set("owner_notice_date", event.target.value)} /></div><div className="space-y-1"><Label>Notice reference</Label><Input value={text("owner_notice_reference")} placeholder="Email subject, letter, receipt…" onChange={(event) => set("owner_notice_reference", event.target.value)} /></div></div>}
      </>}
