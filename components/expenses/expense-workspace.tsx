...
import { ArrowLeft, CalendarDays, CheckCircle2, ExternalLink, XCircle } from "lucide-react"
...
export function ExpenseWorkspace({
  ...
  useEffect(() => {
    ...
    setCostCodeId(selectedExpense.cost_code_id ?? "")
    setQboExpenseAccountId(selectedExpense.qbo_expense_account_id ?? accountingContext?.defaults?.expenseAccountId ?? "")
  ...
  }, [selectedExpense, accountingContext])
  ...
  const save = () => {
    const expenseAccount = findAccount(expenseAccounts, qboExpenseAccountId)
    ...
    startTransition(async () => {
      try {
        await updateProjectExpenseDetailsAction(projectId, selectedExpense.id, {
          ...
          ...(costCodesEnabled ? { costCodeId: costCodeId || null } : {}),
        })
    ...
    <WorkspaceShell open onClose={() => onSelect(null)} listPanel={listPanel} documentPane={documentPane}>
      ...
      <div className="min-h-0 flex-1 space-y-8 overflow-y-auto px-4 py-6 sm:px-6">
        ...
        <section className="space-y-4 rounded-xl border bg-muted/5 p-4">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div className={cn("space-y-1.5", !costCodesEnabled && "sm:col-span-2")}>
            ...
            {costCodesEnabled ? (
              <div className="space-y-1.5">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Cost code</Label>
                <Select value={costCodeId || "__none__"} onValueChange={(value) => setCostCodeId(value === "__none__" ? "" : value)}>
        ...
        {qboConnected ? (
          <section className="space-y-4 rounded-xl border bg-muted/5 p-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-1.5">
                <Label className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Category (account)</Label>
                <Select value={qboExpenseAccountId || undefined} onValueChange={setQboExpenseAccountId}>
                  <SelectTrigger className="h-10 w-full text-sm">
