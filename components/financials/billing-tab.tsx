...
import type { CostCode, Contract, DrawSchedule, Invoice, Project, Retainage, ScheduleItem } from "@/lib/types"
...
interface BillingTabProps {
  ...
  invoices: Invoice[]
  /** Matches across the whole book, not just the loaded page. */
...
export function BillingTab({
  ...
  // Selection is the ?invoice= param and nothing else, so a source flow just has
  // to point the URL at the invoice it produced.
  ...
  useEffect(() => {
    if (invoiceParam) setSubTab("invoices")
  }, [invoiceParam])
      ...
      <Tabs value={subTab} onValueChange={(v) => changeSubTab(v as ReceivablesSubTab)} className="w-full gap-0">
        <TabsContent value="invoices" className="m-0">
          <BillingQueue
            ...
            initialTotalCount={invoiceTotalCount}
            initialCounts={queueCounts as never}
