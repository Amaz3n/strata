...
import { reactivateMemberAction, resetMemberMfaAction, removeMemberAction, resendInviteAction, suspendMemberAction } from "@/app/(app)/team/actions"
...
export function TeamTable({ className, tableWrapperClassName, members, canManageMembers = false, canEditRoles = false, showProjectCounts = true, showInviteAction = true, hideToolbar = false, view: viewProp, onViewChange, onMemberChange, onInviteMember, onEditMember }: TeamTableProps) {
  ...
  const suspend = (membershipId: string) => {
    if (!canManageMembers) {
      toast({
        ...
        description: "You need member management access to suspend users.",
      })
      return
    }
  ...
  const reactivate = (membershipId: string) => {
    if (!canManageMembers) {
      toast({
        title: "Permission required",
        description: "You need member management access to reactivate users.",
      })
      return
    }
  ...
  const remove = (membershipId: string) => {
    if (!canManageMembers) {
      toast({
        title: "Permission required",
        description: "You need member management access to remove users.",
      })
      return
    }
  ...
  const resend = (membershipId: string) => {
    if (!canManageMembers) {
      toast({
        title: "Permission required",
        description: "You need member management access to resend invites.",
      })
      return
    }
  ...
  const resetMfa = (membershipId: string) => {
    if (!canManageMembers) {
      toast({
        title: "Permission required",
        description: "You need member management access to reset MFA.",
      })
      return
    }
  ...
  const activeMembers = useMemo(() => members.filter((member) => member.status !== "suspended"), [members])
  const archivedMembers = useMemo(() => members.filter((member) => member.status === "suspended"), [members])
  ...
  const emptyTitle = view === "archived" ? "No archived members" : members.length > 0 ? "No active members" : "No team members yet"
  const emptyDescription = view === "archived" ? "Archived teammates will appear here." : members.length > 0 ? "Invite or restore a teammate to continue." : "Invite your first team member to get started."
    ...
    <div className={cn("min-w-0 space-y-3", className)}>
      {!hideToolbar && (
        <div className="flex items-center justify-end gap-2">
          ...
          {showInviteAction && (
            <Button size="icon" variant="default" disabled={!canManageMembers} className="h-9 w-9" onClick={() => onInviteMember?.()}>
                ...
                <TableHead className="text-center">
                  <div className="inline-flex items-center justify-center gap-1">
                    ...
                    <Tooltip>
                      ...
                      <TooltipContent>Green lock means MFA is enabled. Red lock means it is not enabled.</TooltipContent>
                    </Tooltip>
                  </div>
                </TableHead>
                ...
                <TableRow key={member.id}>
                  <TableCell className="pl-10">
                    <div className="flex items-center gap-3">
                      <Avatar className="h-9 w-9">
                        ...
                        <AvatarFallback className="text-xs font-medium">{getInitials(member.user.full_name)}</AvatarFallback>
                      </Avatar>
                  ...
                  <TableCell className="text-center">
                    <Tooltip>
                      ...
                      <TooltipContent>{member.mfa_enabled ? "MFA is enabled for this user." : "MFA is not enabled for this user."}</TooltipContent>
                    </Tooltip>
                  </TableCell>
                    ...
                    <TableCell>
                      <div className="flex justify-end pr-10">
                        ...
                        <DropdownMenuContent align="end">
                          ...
                          <DropdownMenuItem disabled={isPending || !canManageMembers || member.status === "invited"} onClick={() => resetMfa(member.id)}>
                            Reset MFA
                          </DropdownMenuItem>
                          <DropdownMenuSeparator />
                          <DropdownMenuItem className="text-destructive" disabled={isPending || !canManageMembers} onClick={() => remove(member.id)}>
                            Remove
                          </DropdownMenuItem>
                        </DropdownMenuContent>
                        </DropdownMenu>
