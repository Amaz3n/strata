...
export function BankReviewTray({ accounts }: { accounts: GlAccount[] }) {
  ...
  const categorize = (row: BankReviewTray["rows"][number], glAccountId: string) => {
  ...
  const exclude = (row: BankReviewTray["rows"][number]) => {
    setBusyId(row.transactionId)
    startTransition(async () => {
      const result = await excludeBankTransactionAction(row.transactionId, row.amountCents)
      setBusyId(null)
      if (!result.success) {
        toast.error(result.error)
        return
  ...
  const confirmAllConfident = (rows: BankReviewTray["rows"]) => {
    const ready = rows.filter((row) => row.best && row.best.confidence >= CONFIDENT)
    startTransition(async () => {
      let matched = 0
  ...
  if (error) {
    return (
      <div className="border bg-background px-5 py-12 text-center">
        <p className="text-sm font-medium">The review tray could not be loaded</p>
  ...
  const confidentCount = data.rows.filter((row) => row.best && row.best.confidence >= CONFIDENT).length
        ...
        <div className="flex items-start gap-2.5 border border-warning/30 bg-warning/10 px-4 py-3">
          ...
          <div className="text-sm">
            <p className="font-medium">
              {data.unmappedAccounts.length === 1 ? "A bank account has" : "Bank accounts have"} no GL
              account mapped
            </p>
            <p className="mt-0.5 text-muted-foreground">
              {data.unmappedAccounts.map((account) => account.name).join(", ")} — transactions here
              cannot be matched to the ledger until a GL account is set. Map{" "}
              {data.unmappedAccounts.length === 1 ? "it" : "them"} in{" "}
              <Link href="/books/banking" className="underline underline-offset-4">
                Banking
              </Link>
              .
            </p>
        ...
        <>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <p className="text-xs text-muted-foreground">
              ...
              {confidentCount > 0 ? ` · ${confidentCount} with a confident suggestion` : null}
            </p>
            {confidentCount > 0 ? (
              <Button size="sm" disabled={pending} onClick={() => confirmAllConfident(data.rows)}>
          ...
          <section className="border bg-background">
            <div className="divide-y">
              {data.rows.map((row) => {
                ...
                return (
                  <div key={row.transactionId} className="grid gap-3 px-4 py-3 sm:grid-cols-[1fr_auto] sm:items-start">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-baseline gap-2">
                        <span className="font-mono text-xs text-muted-foreground">{row.transactionDate}</span>
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        ...
                        {row.best ? (
                          <>
                            {" · "}
                            <span className={cn(row.best.confidence >= CONFIDENT && "text-success")}>
                              suggests {row.best.journalDescription || "a posted ledger line"} on{" "}
                              {row.best.journalDate} ({Math.round(row.best.confidence * 100)}%)
                            </span>
                          </>
                        ) : row.accountUnmapped ? (
                          ...
                          <>
                            {" · "}
                            <span className={cn(row.ruleSuggestion.autoApplies && "text-success")}>
                              a learned rule says {row.ruleSuggestion.accountCode}{" "}
                              {row.ruleSuggestion.accountName}
                              {row.ruleSuggestion.autoApplies ? "" : " (still learning)"}
                            </span>
                          </>
                        ) : (
                      ...
                      {row.alternatives.length > 0 ? (
                        <ul className="mt-1.5 space-y-0.5">
                          {row.alternatives.map((alternative) => (
                            <li key={alternative.journalLineId} className="flex items-center gap-2 text-xs">
                              <button
                                type="button"
                              ...
                              >
                                {alternative.journalDescription || "Posted ledger line"} on{" "}
                                {alternative.journalDate} ({Math.round(alternative.confidence * 100)}%)
                              </button>
                            </li>
                          ))}
                        </ul>
                    ...
                    <div className="flex items-center gap-2 sm:justify-end">
                      <span className="font-mono text-sm tabular-nums">{formatMoneyCentsExact(row.amountCents)}</span>
                      {row.best ? (
                        <Button size="sm" variant="outline" disabled={busy || pending} onClick={() => confirmOne(row)}>
                          Match
                        </Button>
                        ...
                        <Button
                          ...
                          variant="outline"
                          disabled={busy || pending}
                          onClick={() => categorize(row, row.ruleSuggestion!.glAccountId)}
                        >
                        ...
                        <select
                          defaultValue=""
                          disabled={busy || pending}
                          onChange={(event) => categorize(row, event.target.value)}
                          aria-label={`Categorize ${row.counterparty}`}
                          className="flex h-8 max-w-[220px] border border-input bg-transparent px-2 text-xs"
                        >
                      ...
                      <Button size="sm" variant="ghost" disabled={busy || pending} onClick={() => exclude(row)}>
                        Exclude
                      </Button>
                    </div>
                  </div>
