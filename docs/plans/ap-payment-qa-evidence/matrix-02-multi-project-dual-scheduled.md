# MATRIX-02 — Multiple bills across projects, dual approval, scheduled release

Runbook case: `MATRIX-02`
...
- [ ] The frozen batch spans projects, requires two distinct approvers, and does not debit before its scheduled date.
- [ ] Bulk approval of at least five bills from at least three submitters sends exactly one decision notification to each submitter and none to unrelated recipients.
- [ ] An authorized admin other than the preparer can cancel a deliberately stuck approved run; its reservations release and its audit timeline records the reason.
- [ ] Stripe platform payouts are set to `manual`, and approved reserve/platform-balance sweep evidence is linked for the tested hold window.
