# FAILURE-02 — Local update fails after provider acceptance

Runbook case: `FAILURE-02`
...
- [ ] Retry repairs Arc state from the provider idempotency key without moving money twice.
- [ ] A crash after Stripe accepts the vendor transfer but before Arc records the claim is recovered by adoption lookup with exactly one provider transfer and one local claim.
