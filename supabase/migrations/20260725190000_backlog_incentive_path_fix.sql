-- Fix: backlog incentive spend was always zero.
--
-- get_sales_backlog_report read the agreement snapshot at
--   {purchase_agreement,pricing,incentives_cents}
-- but composePurchaseAgreementPricing (lib/financials/purchase-agreement-pricing.ts)
-- serialises camelCase, so the real key is `incentivesCents`. The snake_case path
-- never matched, which meant incentive_spend_cents and incentive_percent_of_price
-- reported 0 for every community regardless of what was actually discounted.
--
-- Reads both spellings so any hand-written or imported snapshot still resolves.
