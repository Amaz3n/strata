-- Phase 0 financial ecosystem reconciliation:
-- align the approved-cost invoice RPC with the TypeScript callers.
--
-- `lib/services/invoices.ts` sends invoice status, client visibility, notes,
-- recipients, and metadata to this RPC. The live/remote schema snapshot only
-- had the older 15-argument function, so the invoice-composer approved-cost
-- path could fail against a migrated database. Drop the old identity and
-- recreate the extended function with defaults so both existing call sites work.

drop function if exists public.create_invoice_from_billable_costs_atomic(
  uuid,
  uuid,
  uuid,
  text,
