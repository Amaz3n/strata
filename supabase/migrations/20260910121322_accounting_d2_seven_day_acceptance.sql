-- User-approved September 10: seven complete UTC dates; all other drop guards remain unchanged.
...
create or replace function public.assert_accounting_d2_ready()
...
begin
  ...
  if c.started_at>(v_today-7)::timestamp at time zone 'UTC' then raise exception 'HOLD D2: seven complete UTC acceptance days have not elapsed'; end if;
  for v_day in select generate_series(v_today-7,v_today-1,'1 day'::interval)::date loop
