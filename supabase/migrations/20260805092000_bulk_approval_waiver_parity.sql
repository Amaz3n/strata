-- Bulk approval now seeds lien-waiver state, like single approval always has.
--
-- `updateVendorBillStatus` sets `lien_waiver_status` on approval — 'requested'
-- when the org requires waivers, 'not_required' when it does not. The bulk RPC
-- skipped it, so two identical payables ended up in different waiver states
-- depending on which button approved them, and bulk-approved bills sat at NULL.
--
-- That gap is load-bearing: `evaluatePaymentHoldFacts` reads
-- `lien_waiver_status` to decide whether the waiver hold blocks release, and
-- `sendVendorBillWaiverChase` only chases bills that were marked 'requested'.
-- A bulk-approved bill was therefore never chased for its waiver.
...
begin
  ...
  -- `vendor_bill_approved` is its own event so the submitter can be told. The
  -- generic `vendor_bill_updated` it used to emit carried no recipient set and
  -- therefore notified nobody.
  insert into public.events (org_id, event_type, entity_type, entity_id, payload, channel)
  select p_org_id, 'vendor_bill_approved', 'vendor_bill', b.id,
    jsonb_build_object('status', 'approved', 'actor_id', p_actor_id, 'bulk', true, 'project_id', b.project_id, 'company_id', b.company_id, 'bill_number', b.bill_number, 'amount_cents', b.total_cents),
    'activity'
  from public.vendor_bills b join jsonb_array_elements(p_items) item on item->>'id' = b.id::text where b.org_id = p_org_id;
