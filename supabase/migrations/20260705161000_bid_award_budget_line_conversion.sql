...
begin
  ...
  if v_package.cost_code_id is not null or v_package.budget_line_id is not null then
...
alter function public.run_bid_award_conversion(uuid, uuid, uuid, text) owner to postgres;

grant all on function public.run_bid_award_conversion(uuid, uuid, uuid, text) to anon;
grant all on function public.run_bid_award_conversion(uuid, uuid, uuid, text) to authenticated;
grant all on function public.run_bid_award_conversion(uuid, uuid, uuid, text) to service_role;
