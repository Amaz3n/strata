...
-- This migration brings a clean database up to the current production schema state
-- DO NOT MODIFY - This captures the complete production schema as of December 2024
...
CREATE TABLE public.contacts (id uuid DEFAULT gen_random_uuid(), org_id uuid, primary_company_id uuid NOT NULL, full_name text, email citext NOT NULL, phone text NOT NULL, role text NOT NULL, contact_type text NOT NULL, metadata jsonb DEFAULT '{}'::jsonb, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now());
...
CREATE TABLE public.estimates (id uuid DEFAULT gen_random_uuid(), org_id uuid, project_id uuid, title text, status text DEFAULT 'draft'::text, version integer DEFAULT 1, subtotal_cents integer NOT NULL, tax_cents integer NOT NULL, total_cents integer NOT NULL, currency text DEFAULT 'usd'::text, metadata jsonb DEFAULT '{}'::jsonb, created_by uuid NOT NULL, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now());
...
CREATE TABLE public.proposals (id uuid DEFAULT gen_random_uuid(), org_id uuid, project_id uuid, estimate_id uuid NOT NULL, recipient_contact_id uuid NOT NULL, status text DEFAULT 'draft'::text, sent_at timestamp with time zone NOT NULL, accepted_at timestamp with time zone NOT NULL, rejected_at timestamp with time zone NOT NULL, snapshot jsonb DEFAULT '{}'::jsonb, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now(), number text NOT NULL, title text NOT NULL, summary text NOT NULL, terms text NOT NULL, valid_until date NOT NULL, total_cents integer NOT NULL, signature_required boolean NOT NULL DEFAULT true, signature_data jsonb NOT NULL, token_hash text NOT NULL, viewed_at timestamp with time zone NOT NULL);
