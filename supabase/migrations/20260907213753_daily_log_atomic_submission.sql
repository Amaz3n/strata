-- Deploy before the application: one retry-safe commit for a log contribution.
-- The authenticated server action checks daily_log.write before invoking this
-- service-only SECURITY INVOKER function. No new authenticated/anon RPC surface.
alter table public.daily_logs add column if not exists submission_id uuid;
alter table public.daily_logs add column if not exists submission_payload jsonb;
create unique index if not exists daily_logs_submission_idx
  on public.daily_logs(org_id, project_id, created_by, submission_id)
...
begin
  ...
  if found then
    ...
    select coalesce(name,address,'a project') into v_project_name from public.projects where id = p_project_id;
...
-- Capture/history reads constrain organization, project and a day/month range.
create index if not exists daily_logs_org_project_date_idx
  on public.daily_logs(org_id, project_id, log_date desc, id);
