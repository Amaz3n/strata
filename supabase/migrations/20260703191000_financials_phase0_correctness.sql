-- Financials Phase 0 correctness hardening.
...
alter table public.time_entries
  drop constraint if exists time_entries_ot_multiplier_check,
  add constraint time_entries_ot_multiplier_check check (ot_multiplier >= 1.0 and ot_multiplier <= 4.0);

alter table public.time_entries
  drop column if exists cost_cents;

alter table public.time_entries
  add column cost_cents integer generated always as (
    round(
      hours
      * base_rate_cents::numeric
      * burden_multiplier
      * case when is_overtime then ot_multiplier else 1 end
    )::integer
  ) stored;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'project_billing_periods_no_overlap'
      and conrelid = 'public.project_billing_periods'::regclass
  ) then
    alter table public.project_billing_periods
      add constraint project_billing_periods_no_overlap
      exclude using gist (
        org_id with =,
        project_id with =,
...
begin
  ...
  v_invoice_total_cents := v_actual_billable_cents - coalesce(v_retainage_cents, 0);
