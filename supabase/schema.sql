...
CREATE TABLE public.entitlements (id uuid DEFAULT gen_random_uuid(), org_id uuid, feature_key text, limit_type text NOT NULL, limit_value numeric NOT NULL, source text DEFAULT 'plan'::text, expires_at timestamp with time zone, created_at timestamp with time zone DEFAULT now());
...
CREATE TABLE public.stripe_connected_accounts (id uuid DEFAULT gen_random_uuid(), org_id uuid NOT NULL, stripe_account_id text NOT NULL, status text DEFAULT 'pending'::text, charges_enabled boolean DEFAULT false, payouts_enabled boolean DEFAULT false, details_submitted boolean DEFAULT false, country text, default_currency text, dashboard_type text, requirement_collection text, onboarding_started_at timestamp with time zone, onboarding_completed_at timestamp with time zone, disabled_reason text, requirements_currently_due jsonb DEFAULT '[]'::jsonb, requirements_eventually_due jsonb DEFAULT '[]'::jsonb, metadata jsonb DEFAULT '{}'::jsonb, created_by uuid, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now());
CREATE TABLE public.webhook_events (id uuid DEFAULT gen_random_uuid(), org_id uuid, provider text NOT NULL, provider_event_id text NOT NULL, event_type text NOT NULL, status text DEFAULT 'received'::text, payload jsonb DEFAULT '{}'::jsonb, processed_at timestamp with time zone, created_at timestamp with time zone DEFAULT now());
...
CREATE TABLE public.subscriptions (id uuid DEFAULT gen_random_uuid(), org_id uuid, plan_code text, status subscription_status DEFAULT 'trialing'::subscription_status, current_period_start timestamp with time zone DEFAULT now(), current_period_end timestamp with time zone, trial_ends_at timestamp with time zone, cancel_at timestamp with time zone, external_customer_id text, external_subscription_id text, created_at timestamp with time zone DEFAULT now(), updated_at timestamp with time zone DEFAULT now());
...
CREATE TRIGGER stripe_connected_accounts_set_updated_at BEFORE UPDATE ON public.stripe_connected_accounts FOR EACH ROW EXECUTE FUNCTION public.tg_set_updated_at();
...
CREATE POLICY stripe_connected_accounts_access ON public.stripe_connected_accounts FOR ALL USING ((auth.role() = 'service_role'::text) OR is_org_member(org_id)) WITH CHECK ((auth.role() = 'service_role'::text) OR is_org_member(org_id));
CREATE POLICY webhook_events_access ON public.webhook_events FOR ALL USING ((auth.role() = 'service_role'::text) OR is_org_member(org_id)) WITH CHECK ((auth.role() = 'service_role'::text) OR is_org_member(org_id));
