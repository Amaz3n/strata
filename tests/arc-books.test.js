...
const { assertBalancedJournalDraft, assertValidOperatingPosture, buildPostingKey } = require("../lib/services/books/types")
const {
  draftFromFact,
  hashableFactPayload,
  isRetiredFactKind,
...
const { resolveRevenueRecognitionBasis } = require("../lib/financials/billing-model")
...
function common(overrides = {}) {
  return { id: "00000000-0000-4000-8000-000000000001", date: "2026-06-30", memo: "Fixture", projectionVersion: 1, policyVersion: 1, ...overrides }
}

test("Arc Books golden postings are balanced in integer cents", () => {
  const entries = [
    postVendorBillFromCostLines({ ...common(), grossCents: 125000, retainageCents: 12500, costLines: [{ amountCents: 125000 }] }),
    postVendorBillFromCostLines({ ...common(), grossCents: 100000, costLines: [{ amountCents: 60000 }, { amountCents: 40000 }] }),
    postBillPayment({ ...common(), amountCents: 112500 }),
    postBillPayment({ ...common(), amountCents: 112500, feeCents: 250, discountCents: 2000 }),
    ...
    postVendorBillFromCostLines({ ...common(), grossCents: -40000, costLines: [{ amountCents: -40000 }] }),
    postExpense({ ...common(), amountCents: -3599 }),
    postLaborCost({ ...common(), amountCents: -12000 }),
    postRetainageRelease({ ...common(), amountCents: 12500, side: "payable" }),
    postRetainageRelease({ ...common(), amountCents: 25000, side: "receivable" }),
    postLaborCost({ ...common(), amountCents: 50000 }),
    postRevenueRecognition({ ...common(), projectId: PROJECT_ID, deltaCents: 44000, periodKey: "2026-06" }),
    postRevenueRecognition({ ...common(), projectId: PROJECT_ID, deltaCents: -12000, periodKey: "2026-07" }),
    postYearEndClose({
      ...common(),
  ...
  assert.deepEqual(
    postVendorBillFromCostLines({ ...common(), grossCents: 125000, retainageCents: 12500, costLines: [{ amountCents: 125000 }] }).lines.map((line) => [line.accountCode, line.debitCents, line.creditCents]),
    [
      ["5000", 125000, 0],
      ["2000", 0, 112500],
      ["2010", 0, 12500],
...
test("billing a customer is not revenue; percentage-of-completion recognizes it separately", () => {
  const invoice = postCustomerInvoice({ ...common(), projectId: PROJECT_ID, grossCents: 250000, retainageCents: 25000 })
  // AR + retainage receivable against contract liabilities — never 4000 directly.
  assert.deepEqual(
    invoice.lines.map((line) => [line.accountCode, line.debitCents, line.creditCents]),
    [
      ["1100", 225000, 0],
      ["1110", 25000, 0],
      ["2350", 0, 250000],
  ...
  const earned = postRevenueRecognition({ ...common(), projectId: PROJECT_ID, deltaCents: 180000, periodKey: "2026-06" })
  assert.deepEqual(
    earned.lines.map((line) => [line.accountCode, line.debitCents, line.creditCents]),
    [
      ["2350", 180000, 0],
      ["4000", 0, 180000],
    ],
  ...
  const reversal = postRevenueRecognition({ ...common(), projectId: PROJECT_ID, deltaCents: -20000, periodKey: "2026-07" })
  assert.deepEqual(
    reversal.lines.map((line) => [line.accountCode, line.debitCents, line.creditCents]),
    [
      ["4000", 20000, 0],
      ["2350", 0, 20000],
    ],
...
test("closing-basis projects book revenue at the sale instead of percentage-of-completion", () => {
  assert.equal(resolveRevenueRecognitionBasis({ ownerBillingBasis: "closing" }), "closing")
  assert.equal(resolveRevenueRecognitionBasis({ ownerBillingBasis: "draws" }), "percentage_of_completion")
  assert.equal(resolveRevenueRecognitionBasis({ ownerBillingBasis: "costs_plus_fee" }), "percentage_of_completion")
  const closing = postClosingInvoice({ ...common(), projectId: PROJECT_ID, grossCents: 425000 })
  assert.deepEqual(
    closing.lines.map((line) => [line.accountCode, line.debitCents, line.creditCents]),
    [
      ["1100", 425000, 0],
      ["4000", 0, 425000],
    ],
...
test("posting keys embed the projection and source versions so re-projection cannot collide", () => {
  assert.equal(buildPostingKey("vendor_bill:abc", { projectionVersion: 1 }), "vendor_bill:abc:s1:v1")
  assert.equal(buildPostingKey("vendor_bill:abc", { projectionVersion: 2, sourceVersion: 3 }), "vendor_bill:abc:s3:v2")
  assert.throws(() => buildPostingKey("x", { projectionVersion: 0 }), /positive integer/)
  assert.throws(() => buildPostingKey("x", { projectionVersion: 1, sourceVersion: 0 }), /positive integer/)

  const v1 = postVendorBillFromCostLines({ ...common(), grossCents: 1000, projectionVersion: 1, costLines: [{ amountCents: 1000 }] })
  const v2 = postVendorBillFromCostLines({ ...common(), grossCents: 1000, projectionVersion: 2, costLines: [{ amountCents: 1000 }] })
  const revised = postVendorBillFromCostLines({ ...common(), grossCents: 1000, projectionVersion: 1, sourceVersion: 2, costLines: [{ amountCents: 1000 }] })
...
test("a fact rebuilds to the identical draft, and lifecycle columns are not economic", () => {
  const payload = {
    ...
    company_id: null,
    cost_lines: [{ amount_cents: 80000, project_id: PROJECT_ID }, { amount_cents: 45000, project_id: PROJECT_ID }],
  }
  const input = {
  ...
  assert.equal(first.lines.filter((line) => line.accountCode === "5000").length, 2)

  // A memo edit must not read as an economic revision; an amount change must.
  assert.deepEqual(hashableFactPayload({ ...payload, memo: "renamed" }), hashableFactPayload(payload))
  assert.notDeepEqual(hashableFactPayload({ ...payload, total_cents: 999 }), hashableFactPayload(payload))
})
...
test("retainage: an invoice total is net, a vendor bill total is gross", () => {
  ...
  const amounts = (draft) =>
    Object.fromEntries(
      draft.lines.map((line) => [line.accountCode, line.debitCents > 0 ? line.debitCents : -line.creditCents]),
    )
  ...
  const invoice = draftFromFact({
    ...
    accountingDate: "2026-06-30",
    payload: { memo: "Invoice 12", total_cents: 225000, retainage_cents: 25000, project_id: PROJECT_ID },
    sourceVersion: 1,
    projectionVersion: 1,
    policyVersion: 1,
  })
  assert.deepEqual(amounts(invoice), { 1100: 225000, 1110: 25000, 2350: -250000 })
  ...
  assert.deepEqual(amounts(bill), { 5000: 125000, 2000: -112500, 2010: -12500 })
  ...
  const plain = draftFromFact({
    ...
    accountingDate: "2026-06-30",
    payload: { memo: "Invoice 13", total_cents: 90000, retainage_cents: 0, project_id: PROJECT_ID },
    sourceVersion: 1,
    projectionVersion: 1,
    policyVersion: 1,
  })
...
test("Arc Books honors the expense account selected on each payable line", () => {
  ...
  assert.deepEqual(
    draft.lines.map((line) => [line.accountCode, line.debitCents, line.creditCents]),
    [
      ["5100", 80000, 0],
      ["5200", 45000, 0],
      ["2000", 0, 125000],
...
test("C1 acceptance: a full contract lifecycle projects deterministically and balances", () => {
  ...
  const lifecycle = [
    ...
    fact("bill_payment", "00000000-0000-4000-8000-00000000c002", { memo: "Pay 1041", amount_cents: 112500 }),
    fact("retainage_release", "00000000-0000-4000-8000-00000000c003", {
      memo: "AP retainage release", amount_cents: 12500, side: "payable", project_id: PROJECT_ID,
    }),
    ...
    fact("invoice", "00000000-0000-4000-8000-00000000c004", {
      memo: "Invoice 12", total_cents: 225000, retainage_cents: 25000, project_id: PROJECT_ID,
    }),
    fact("invoice_payment", "00000000-0000-4000-8000-00000000c005", { memo: "Receipt", amount_cents: 225000 }),
    fact("retainage_release", "00000000-0000-4000-8000-00000000c006", {
      memo: "AR retainage release", amount_cents: 25000, side: "receivable", project_id: PROJECT_ID,
    }),
    fact("expense", "00000000-0000-4000-8000-00000000c007", {
      memo: "Dump fees", amount_cents: 3599, project_id: PROJECT_ID,
    }),
    fact("labor_cost", "00000000-0000-4000-8000-00000000c008", {
      memo: "Field crew", amount_cents: 50000, project_id: PROJECT_ID,
    }),
    fact("payment_reversal", "00000000-0000-4000-8000-00000000c009", {
      memo: "ACH return", amount_cents: 10000, side: "invoice_payment", project_id: PROJECT_ID,
    }),
  ]

  const firstPass = lifecycle.map(draftFromFact)
  const secondPass = lifecycle.map(draftFromFact)
  assert.ok(firstPass.every(Boolean), "every lifecycle fact must produce a draft")
  ...
  const debitsTo = (code) =>
    firstPass.flatMap((draft) => draft.lines).filter((line) => line.accountCode === code)
...
test("a retainage release moves a balance instead of booking new cost or new billing", () => {
  ...
  const draft = (side, amountCents) =>
    draftFromFact({
      ...
      accountingDate: "2026-06-30",
      payload: { memo: "Retainage release", amount_cents: amountCents, side, project_id: PROJECT_ID },
      sourceVersion: 1,
      projectionVersion: 1,
      policyVersion: 1,
    })
  ...
  assert.deepEqual(
    payable.lines.map((line) => [line.accountCode, line.debitCents, line.creditCents]),
    [
      ["2010", 12500, 0],
      ["2000", 0, 12500],
    ],
  )
  assert.ok(!payable.lines.some((line) => line.accountCode === "5000"), "must not debit job costs again")

  const receivable = draft("receivable", 25000)
  assert.deepEqual(
    receivable.lines.map((line) => [line.accountCode, line.debitCents, line.creditCents]),
    [
      ["1100", 25000, 0],
      ["1110", 0, 25000],
    ],
  )
  assert.ok(!receivable.lines.some((line) => line.accountCode === "2350"), "must not re-credit contract liabilities")
  ...
  const projectorSource = fs.readFileSync(path.join(__dirname, "../lib/services/books/projector.ts"), "utf8")
  assert.match(projectorSource, /source === "retainage_release"/)
...
test("every ledger tie-out has a matching spine category excluded from the close gate", () => {
  ...
  const returnedBlock = verifierSource.slice(verifierSource.indexOf("export async function runLedgerTieOuts"))
  const codes = [...returnedBlock.matchAll(/^\s{6}code: "([a-z_]+)",$/gm)].map((match) => match[1])
  assert.ok(codes.length >= 7, `expected the full tie-out set, found ${codes.length}`)
  ...
  const categories = [...categoryBlock.matchAll(/"([a-z_]+)"/g)].map((match) => match[1])
...
test("the projector never consumes the payment rails subledger", () => {
  ...
  assert.match(railsSource, /not\*\* a general ledger|\*\*not\*\* a general ledger/)
...
test("year-end close classifies by account type, not by code prefix", () => {
  ...
  assert.throws(
    () => postYearEndClose({ ...common(), incomeAccountBalances: [{ accountCode: "1000", accountType: "asset", balanceCents: 100 }] }),
    /cannot be closed/,
  )
})
...
test("year-end close survives a contra balance on either side", () => {
  ...
  assert.equal(revenue.creditCents, 30000, "an income account with a debit balance closes with a credit")
  const cogs = draft.lines.find((line) => line.accountCode === "5000")
  assert.equal(cogs.debitCents, 12000, "an expense account with a credit balance closes with a debit")
  ...
  assert.equal(breakEven.lines.find((line) => line.accountCode === "3100"), undefined)
...
test("cost lines must account for the whole bill", () => {
  assert.throws(
    () => postVendorBillFromCostLines({ ...common(), grossCents: 100000, costLines: [{ amountCents: 60000 }] }),
...
function legacyWipRow(input) {
  const completion = input.eacCents > 0
    ? Math.min(1, Math.max(0, input.actualCostCents / input.eacCents))
    : 0
  const earnedRevenueCents = Math.round(input.revisedContractCents * completion)
...
test("the shared POC rule reproduces the pre-extraction WIP numbers exactly", () => {
  const cases = [
    { name: "ordinary mid-job", originalContractCents: 1_000_000, approvedChangeOrdersCents: 100_000, revisedContractCents: 1_100_000, actualCostCents: 400_000, eacCents: 800_000, billedCents: 600_000 },
    { name: "no contract value", originalContractCents: 0, approvedChangeOrdersCents: 0, revisedContractCents: 0, actualCostCents: 250_000, eacCents: 500_000, billedCents: 0 },
    { name: "no EAC — division guarded", originalContractCents: 500_000, approvedChangeOrdersCents: 0, revisedContractCents: 500_000, actualCostCents: 120_000, eacCents: 0, billedCents: 90_000 },
    { name: "overrun past 100% — completion clamps", originalContractCents: 800_000, approvedChangeOrdersCents: 0, revisedContractCents: 800_000, actualCostCents: 950_000, eacCents: 900_000, billedCents: 800_000 },
    { name: "job at a forecast loss", originalContractCents: 400_000, approvedChangeOrdersCents: 0, revisedContractCents: 400_000, actualCostCents: 300_000, eacCents: 620_000, billedCents: 410_000 },
    { name: "deductive change order", originalContractCents: 900_000, approvedChangeOrdersCents: -150_000, revisedContractCents: 750_000, actualCostCents: 300_000, eacCents: 600_000, billedCents: 300_000 },
    { name: "rounding — odd cents", originalContractCents: 333_333, approvedChangeOrdersCents: 1, revisedContractCents: 333_334, actualCostCents: 100_001, eacCents: 300_007, billedCents: 111_111 },
    { name: "untouched job", originalContractCents: 250_000, approvedChangeOrdersCents: 0, revisedContractCents: 250_000, actualCostCents: 0, eacCents: 250_000, billedCents: 0 },
  ]

  for (const { name, ...input } of cases) {
    const legacy = legacyWipRow(input)
    const shared = computeProjectPoc(input)
    assert.equal(shared.earnedRevenueCents, legacy.earnedRevenueCents, `${name}: earned revenue`)
    assert.equal(shared.overUnderCents, legacy.overUnderCents, `${name}: over/under`)
    assert.equal(shared.costToCompleteCents, legacy.costToCompleteCents, `${name}: cost to complete`)
    assert.equal(shared.forecastGrossProfitCents, legacy.forecastGrossProfitCents, `${name}: gross profit`)
    assert.equal(shared.forecastGrossMarginPercent, legacy.forecastGrossMarginPercent, `${name}: gross margin`)
    // The report displays a percentage; the rule stores the ratio it came from.
    assert.equal(Math.round(shared.completionRatio * 1000) / 10, legacy.percentDisplayed, `${name}: percent complete`)
    // Money stays integer cents through every path.
    for (const key of ["earnedRevenueCents", "overUnderCents", "costToCompleteCents", "forecastGrossProfitCents"]) {
      assert.ok(Number.isSafeInteger(shared[key]), `${name}: ${key} is integer cents`)
    }
  }
})
...
test("POC input resolution has one definition for the snapshot and the report", () => {
  ...
  assert.equal(
    resolveRevisedContractCents({
      billingContract: { snapshot: { revised_total_cents: 1_100_000 }, total_cents: 900_000 },
      totalContractValueCents: 500_000,
    }),
    1_100_000,
  )
  assert.equal(resolveRevisedContractCents({ billingContract: null, totalContractValueCents: 500_000 }), 500_000)
  assert.equal(resolveRevisedContractCents({ billingContract: null, totalContractValueCents: null }), 0)
  ...
  assert.equal(
    resolveOriginalContractCents({ billingContract: null, revisedContractCents: 100_000, approvedChangeOrdersCents: 120_000 }),
    100_000,
  )
  assert.equal(
    resolveOriginalContractCents({ billingContract: null, revisedContractCents: 1_100_000, approvedChangeOrdersCents: 100_000 }),
    1_000_000,
  )
  assert.equal(
    resolveOriginalContractCents({
  ...
  assert.equal(resolveEacCents({ summaryEacCents: 800_000, adjustedBudgetCents: 700_000, actualCostCents: 400_000 }), 800_000)
  assert.equal(resolveEacCents({ summaryEacCents: 0, adjustedBudgetCents: 500_000, actualCostCents: 640_000 }), 640_000)
...
test("missing_budget reaches the POC result instead of dying in the report", () => {
  const withBudget = computeProjectPoc({
    originalContractCents: 500_000, approvedChangeOrdersCents: 0, revisedContractCents: 500_000,
    actualCostCents: 100_000, eacCents: 400_000, billedCents: 120_000,
  })
  assert.deepEqual(withBudget.warnings, [])

  const withoutBudget = computeProjectPoc({
    originalContractCents: 500_000, approvedChangeOrdersCents: 0, revisedContractCents: 500_000,
    actualCostCents: 0, eacCents: 0, billedCents: 0,
  ...
  const deduped = computeProjectPoc({
    originalContractCents: 0, approvedChangeOrdersCents: 0, revisedContractCents: 0,
    actualCostCents: 0, eacCents: 0, billedCents: 0,
  }, { extraWarnings: ["missing_contract_value"] })
  assert.equal(deduped.warnings.filter((w) => w === "missing_contract_value").length, 1)
})
...
test("POC journals are review-only month-boundary exports", () => {
  ...
  assert.match(source, /accountLabel\(SYSTEM_ACCOUNT_CODES\.contractLiability\)/)
  ...
  assert.doesNotMatch(source, /AccountingExportKind = "ap" \| "job_cost" \| "journal"/)
...
test("the ledger authority gate has one implementation and fails closed", () => {
  const authority = fs.readFileSync(path.join(__dirname, "../lib/services/books/authority.ts"), "utf8")
  const sync = fs.readFileSync(path.join(__dirname, "../lib/services/accounting-sync.ts"), "utf8")
  const qboInbound = fs.readFileSync(path.join(__dirname, "../lib/integrations/accounting/qbo/reconcile.ts"), "utf8")
  ...
  assert.match(authority, /data\?\.ledger_authority === "arc" \? "arc" : "external"/)
  ...
  assert.match(qboInbound, /resolveLedgerAuthority\(connection\.org_id, supabase\)/)
})
...
test("AR and AP have one status definition, and it is the one the GL posts from", () => {
  ...
  assert.deepEqual([...BILLED_INVOICE_STATUSES], ["sent", "partial", "paid", "overdue"])
  assert.deepEqual([...PAYABLE_VENDOR_BILL_STATUSES], ["approved", "partial", "paid"])
  ...
  for (const relative of seam) {
    ...
    assert.doesNotMatch(source, /"approved",\s*"partial",\s*"paid"/, `${relative} re-declares the AP set`)
  }
  ...
  const arAging = fs.readFileSync(path.join(__dirname, "../lib/services/reports/ar-aging.ts"), "utf8")
  const apAging = fs.readFileSync(path.join(__dirname, "../lib/services/reports/ap-aging.ts"), "utf8")
  assert.match(arAging, /\.in\("status", \[\.\.\.BILLED_INVOICE_STATUSES\]\)/)
  assert.match(apAging, /\.in\("status", \[\.\.\.PAYABLE_VENDOR_BILL_STATUSES\]\)/)
  ...
  const verifier = fs.readFileSync(path.join(__dirname, "../lib/services/books/verifier.ts"), "utf8")
  ...
  assert.match(verifier, /invoice:invoices!invoice_id!inner\(status, issue_date\)/)
  assert.match(verifier, /\.in\("invoice\.status", \[\.\.\.BILLED_INVOICE_STATUSES\]\)/)
...
test("the journal balance guard resolves its entry id with control flow, not a CASE expression", () => {
  ...
  assert.ok(defining.length > 0, "no migration defines books_assert_journal_balanced")

  const winner = fs.readFileSync(path.join(dir, defining[defining.length - 1]), "utf8")
  const body = winner.slice(winner.search(/create or replace function public\.books_assert_journal_balanced/i))
  assert.match(body, /if tg_table_name = 'journal_entries' then\s*\n\s*target_entry_id := new\.id;/i)
  ...
  assert.match(body, /elsif tg_op = 'DELETE' then\s*\n\s*target_entry_id := old\.entry_id;/i)
})

test("cash flow splits a movement across every counterpart, not just the largest", () => {
  const { allocateCashMovement } = require("../lib/services/books/cash-flow-rules")
  const total = (allocation) => allocation.operating + allocation.investing + allocation.financing
  ...
  const single = allocateCashMovement(250_000, [{ weightCents: 250_000, category: "investing" }])
  ...
  assert.equal(Math.max(thirds.operating, thirds.investing, thirds.financing), 34)
  ...
  assert.equal(total(allocateCashMovement(5_000, [{ weightCents: 5_000, category: null }])), 5_000)
  assert.equal(allocateCashMovement(5_000, [{ weightCents: 5_000, category: null }]).operating, 5_000)
  assert.equal(allocateCashMovement(7_777, []).operating, 7_777)
  assert.equal(total(allocateCashMovement(0, [{ weightCents: 100, category: "operating" }])), 0)

  // Zero-weight lines cannot absorb a share and must not divide by zero.
  assert.equal(total(allocateCashMovement(-3_333, [{ weightCents: 0, category: "financing" }])), -3_333)
  ...
  for (const movement of [1, -1, 999_999, -123_457, 50_505]) {
    ...
    assert.equal(total(allocation), movement, `allocation must sum to ${movement}`)
  }
})

test("the P&L carries the project dimension and drill-down reuses the shared href templates", () => {
  const statements = fs.readFileSync(path.join(__dirname, "../lib/services/books/statements.ts"), "utf8")
  const detail = fs.readFileSync(path.join(__dirname, "../lib/services/books/statement-detail.ts"), "utf8")
  ...
  assert.match(detail, /from\("journal_entries"\)[\s\S]*?lines:journal_lines!inner/)
...
test("the journal editor replaced the raw-JSON adjustment form", () => {
  ...
  const journals = fs.readFileSync(path.join(__dirname, "../components/books/books-journals.tsx"), "utf8")
  const actions = fs.readFileSync(path.join(__dirname, "../app/(app)/books/actions.ts"), "utf8")
...
test("cash-basis conversion reproduces the cash that actually moved", () => {
  const { convertToCashBasis } = require("../lib/services/books/cash-basis-rules")
  ...
  assert.deepEqual(settled.revenueAdjustments, [], "no movement means no adjustment lines")
  ...
  for (const statement of [qa, deposit]) {
    const revenueDelta = statement.revenueAdjustments.reduce((sum, item) => sum + item.amountCents, 0)
    const costDelta = statement.costAdjustments.reduce((sum, item) => sum + item.amountCents, 0)
    assert.equal(statement.accrualRevenueCents + revenueDelta, statement.cashReceiptsCents)
    assert.equal(statement.accrualCostCents + costDelta, statement.cashPaidCents)
  }
})

test("bank matching respects direction, the date window, and lines already taken", () => {
  const { rankBankMatches, MATCH_WINDOW_DAYS } = require("../lib/services/books/bank-match-rules")
  const outflow = {
    id: "txn-1",
    date: "2026-07-01",
  ...
  const directional = rankBankMatches({
    transaction: outflow,
    candidates: [line("credit-line"), line("debit-line", { debitCents: 9_000_000, creditCents: 0 })],
  })
  ...
  assert.equal(rankBankMatches({ transaction: outflow, candidates: [line("x", { creditCents: 8_999_999 })] }).length, 0)

  // Outside the window it is not the same event, however well it scores inside it.
  const justOutside = new Date(Date.UTC(2026, 6, 1) + (MATCH_WINDOW_DAYS + 1) * 86_400_000)
    .toISOString()
    .slice(0, 10)
  assert.equal(rankBankMatches({ transaction: outflow, candidates: [line("x", { entryDate: justOutside })] }).length, 0)
  ...
  assert.ok(ranked.every((match) => match.confidence >= 0 && match.confidence <= 1))
})
...
test("bank rules pick the most specific match and reuse B1's confidence curve", () => {
  ...
  const { CODING_RULE_AUTO_APPLY_HITS } = require("../lib/services/accounting-rules")
  ...
  assert.equal(
    selectBankRule({
      transaction: txn,
      rules: [rule({ id: "substring" }), rule({ id: "exact", matchKind: "merchant_exact", matchValue: "home depot" })],
    }).ruleId,
    "exact",
  )
  ...
  assert.equal(
    selectBankRule({
      transaction: txn,
      rules: [rule({ id: "any-account" }), rule({ id: "this-account", bankAccountId: "acct-1" })],
    }).ruleId,
    "this-account",
  )
  ...
  assert.equal(
    selectBankRule({
      transaction: { ...txn, merchantName: null, description: "home depot pro desk" },
      rules: [rule({ id: "short", matchValue: "home depot" }), rule({ id: "long", matchValue: "home depot pro" })],
    }).ruleId,
    "long",
  )

  // Scope is respected: wrong account, wrong direction, or inactive means no match.
  assert.equal(selectBankRule({ transaction: txn, rules: [rule({ bankAccountId: "other" })] }), null)
  assert.equal(selectBankRule({ transaction: txn, rules: [rule({ direction: "inflow" })] }), null)
  assert.equal(selectBankRule({ transaction: txn, rules: [rule({ active: false })] }), null)

  // Auto-apply is B1's threshold, not a second one invented here.
  assert.equal(selectBankRule({ transaction: txn, rules: [rule({ hitCount: CODING_RULE_AUTO_APPLY_HITS })] }).autoApplies, true)
  assert.equal(selectBankRule({ transaction: txn, rules: [rule({ hitCount: CODING_RULE_AUTO_APPLY_HITS - 1 })] }).autoApplies, false)
  ...
  assert.deepEqual(buildBankRuleLesson({ merchantName: "Home Depot", description: "HOME DEPOT #4821" }), {
    matchKind: "merchant_exact",
    matchValue: "home depot",
  })
  assert.equal(buildBankRuleLesson({ merchantName: null, description: "ACH" }), null)
  assert.deepEqual(buildBankRuleLesson({ merchantName: null, description: "SUNBELT RENTALS" }), {
    matchKind: "description_contains",
  ...
  assert.equal(isBankRuleCorrection({ applied: null, final: { glAccountId: "gl-a" } }), false)
  assert.equal(isBankRuleCorrection({ applied: { glAccountId: "gl-a" }, final: { glAccountId: "gl-a" } }), false)
  assert.equal(isBankRuleCorrection({ applied: { glAccountId: "gl-a" }, final: { glAccountId: "gl-b" } }), true)
})

test("a pasted trial balance parses without inventing amounts", () => {
  const { parseTrialBalance } = require("../lib/financials/trial-balance-import")
  const accounts = [
    { id: "a", code: "1000", name: "Operating cash", account_type: "asset", active: true },
    { id: "b", code: "1100", name: "Accounts receivable", account_type: "asset", active: true },
    { id: "c", code: "3000", name: "Owner equity", account_type: "equity", active: true },
  ]

  // Column-aligned with separate debit and credit columns — the common export.
  const columns = parseTrialBalance(
    ["1000  Operating cash        125,000.00   0.00", "3000  Owner equity          0.00         125,000.00"].join("\n"),
    accounts,
  )
  ...
  const signed = parseTrialBalance(["1000,Operating cash,125000.00", "3000,Owner equity,-125000.00"].join("\n"), accounts)
  ...
  const unknown = parseTrialBalance("9999  Some other ledger  100.00", accounts)
  ...
  assert.equal(parseTrialBalance("\n\n  \n1000 Operating cash 100.00\n\n", accounts).length, 1)
})
...
test("disabling Arc Books preserves ledger standing instead of demoting to shadow", () => {
  ...
  const disableUpdate = /\.update\(\{\s*\n\s*workspace_enabled: false,(?<fields>[\s\S]*?)\}\)/.exec(moduleService)
  assert.ok(disableUpdate, "the disable path must update workspace_enabled to false")
  ...
  assert.match(cutover, /settings\.data\?\.workspace_enabled === true &&\s*\n\s*settings\.data\?\.arc_ledger_mode === "parallel"/)
})
...
test("the cutover quarter gate measures calendar covered, not runs approved", () => {
  const { comparisonSpanDays, hasQuarterOfSilentCorrectness } = require("../lib/services/books/cutover-rules")
...
test("the cutover run records the quarter gate as a first-class prerequisite", () => {
  ...
  assert.match(cutover, /quarter_of_silent_correctness: hasQuarterOfSilentCorrectness\(comparedPeriods\)/)
  assert.match(cutover, /period:accounting_periods\(period_start, period_end\)/)
})
...
test("a corrected coding rule is demoted, not retired, and can earn auto-apply back", () => {
  ...
  for (let i = 0; i < 3; i += 1) counts = nextCodingRuleCounts({ ...counts, corrected: false })
  ...
  for (let i = 0; i < 3; i += 1) recovering = nextCodingRuleCounts({ ...recovering, corrected: false })
...
test("a once-corrected rule auto-applies again after its cooldown, and not before", () => {
  const { CODING_RULE_COOLDOWN_DAYS } = require("../lib/services/accounting-rules")
  const rule = {
    id: "recovered",
    company_id: "vendor-1",
    match_kind: "vendor",
  ...
  const select = (now) => selectCodingSuggestion({ companyId: "vendor-1", now: new Date(now), rules: [rule] })
...
test("only a changed coding value counts as contradicting the rule", () => {
  ...
  assert.equal(isCodingCorrection({ applied, final: { costCodeId: "cc-1", budgetLineId: "bl-1" } }), false)
  assert.equal(isCodingCorrection({ applied, final: { costCodeId: "cc-2", budgetLineId: "bl-1" } }), true)
  assert.equal(isCodingCorrection({ applied, final: { costCodeId: "cc-1", budgetLineId: null } }), true)

  // No rule coded it, so there is nothing to contradict — this is a fresh lesson.
  assert.equal(isCodingCorrection({ applied: null, final: { costCodeId: "cc-9", budgetLineId: null } }), false)
})

test("coding provenance and timestamps survive a learn", () => {
  const service = fs.readFileSync(path.join(__dirname, "../lib/services/books/coding-rules.ts"), "utf8")
  ...
  assert.match(service, /last_hit_at: corrected \? existing\?\.last_hit_at \?\? null : now/)
  assert.match(service, /last_corrected_at: corrected \? now : existing\?\.last_corrected_at \?\? null/)
  // Provenance is set once, at birth.
  assert.match(service, /created_from: existing\?\.created_from \?\? "user_correction"/)
  // The concurrent-learn race is closed by upserting on the natural key.
  assert.match(service, /onConflict: "org_id,match_kind,company_id,match_value,memo_pattern"/)
...
test("coding rules declare only the match kinds and provenance that exist", () => {
  const migration = fs.readFileSync(path.join(__dirname, "../supabase/migrations/20260808150000_coding_rule_enum_cleanup.sql"), "utf8")
  const pure = fs.readFileSync(path.join(__dirname, "../lib/services/accounting-rules.ts"), "utf8")
  const service = fs.readFileSync(path.join(__dirname, "../lib/services/books/coding-rules.ts"), "utf8")
  ...
  assert.match(migration, /check \(match_kind in \('vendor', 'vendor_memo'\)\)/)
...
test("a seeded discrepancy turns a tie-out red and clears when it is cured", () => {
  ...
  const tieOut = (ledgerCents, subledgerCents) => {
    ...
    return { status: differenceCents === 0 ? "passed" : "failed", differenceCents }
  }

  assert.deepEqual(tieOut(1_250_00, 1_250_00), { status: "passed", differenceCents: 0 })
  assert.deepEqual(tieOut(1_250_01, 1_250_00), { status: "failed", differenceCents: 1 })
  assert.deepEqual(tieOut(1_250_00, 1_250_01), { status: "failed", differenceCents: -1 })
...
test("the close band names every gate and points each failure at its cure", () => {
  const periodClose = fs.readFileSync(path.join(__dirname, "../lib/services/books/period-close.ts"), "utf8")
  ...
  for (const code of ["sync_backlog", "waiver_holds", "retainage_movement"]) {
    assert.match(periodClose, new RegExp(`code: "${code}"`), `close band is missing ${code}`)
  }
  ...
  // read as a clean one.
  assert.match(periodClose, /scan_capped: unpushed\.length >= SYNC_BACKLOG_SCAN_LIMIT/)

  // A checklist row that names a failure without saying where to cure it makes
  // the person closing the period go hunting.
  assert.match(periodClose, /evidence: check\.href \? \{ \.\.\.check\.evidence, href: check\.href \} : check\.evidence/)
  ...
  assert.match(periodClose, /\.lt\("released_at", exclusiveDayAfter\(period\.period_end\)\)/)
})

test("org-level notifications can deep-link even with no project", () => {
  const events = fs.readFileSync(path.join(__dirname, "../lib/services/events.ts"), "utf8")
  const item = fs.readFileSync(path.join(__dirname, "../components/notifications/notification-item.tsx"), "utf8")
  ...
  assert.doesNotMatch(events, /metadata: \{ href: "\/reports\/accounting-reconciliation" \}/)
  ...
  assert.match(item, /const explicit = typeof payload\?\.href === "string" \? payload\.href : null/)
  ...
  assert.match(emailDelivery, /if \(typeof payload\.href === "string" && payload\.href\.startsWith\("\/"\)\) return payload\.href/)
})

test("connection findings describe connections the org still means to use", () => {
  const source = fs.readFileSync(path.join(__dirname, "../lib/services/books/reconciliation.ts"), "utf8")
  ...
  assert.match(source, /RECONCILABLE_CONNECTION_STATUSES = \["active", "expired", "error"\]/)
  assert.match(source, /\.in\("status", \[\.\.\.RECONCILABLE_CONNECTION_STATUSES\]\)/)
  ...
  assert.match(source, /connection\.last_inbound_poll_at \? new Date\(connection\.last_inbound_poll_at\)\.getTime\(\) : 0/)
  ...
  const unhealthyBranch = source.slice(source.indexOf('category: "connection_unhealthy"'))
  assert.ok(
    unhealthyBranch.indexOf("continue") < unhealthyBranch.indexOf('category: "connection_stale"'),
...
test("the external mirror summarizes a period per account instead of replaying transactions", () => {
  const { buildMirrorSummary, mirrorReference } = require("../lib/services/books/mirror-rules")
  ...
  const mappings = [
    { glAccountId: "a-cash", externalAccountId: "qb-1", externalAccountName: "Checking" },
    { glAccountId: "a-ar", externalAccountId: "qb-2", externalAccountName: "A/R" },
    { glAccountId: "a-rev", externalAccountId: "qb-3", externalAccountName: "Income" },
    { glAccountId: "a-quiet", externalAccountId: "qb-9", externalAccountName: "Other" },
  ]
  ...
  const summary = buildMirrorSummary({ lines, accounts, mappings, periodLabel: "FY2026 P7" })
  assert.equal(summary.ok, true)
  assert.equal(summary.lines.length, 3, "one line per account with activity, and none for a net-zero account")
  assert.equal(summary.totalDebitCents, summary.totalCreditCents)
  assert.deepEqual(
    summary.lines.map((line) => [line.externalAccountId, line.debitCents, line.creditCents]),
    [
      ["qb-1", 30_000, 0],   // cash
  ...
  assert.deepEqual(missingMapping.unmappedAccounts.map((account) => account.code), ["4000"])
  ...
  assert.equal(mirrorReference("period-1", "conn-1"), "books_period_summary:period-1:conn-1")
  assert.notEqual(mirrorReference("period-1", "conn-1"), mirrorReference("period-2", "conn-1"))
})
...
test("the accounting layer is provider-neutral outward from the interface", () => {
  ...
  const sheet = fs.readFileSync(path.join(__dirname, "../components/integrations/accounting-sync-sheet.tsx"), "utf8")
...
function pagedReadsWithoutOrder(relativePath) {
  const source = withoutComments(fs.readFileSync(path.join(__dirname, relativePath), "utf8"))
  const offenders = []
  let cursor = source.indexOf(".range(")
  while (cursor !== -1) {
    const chainStart = source.lastIndexOf(".from(", cursor)
    const chain = chainStart === -1 ? source.slice(0, cursor) : source.slice(chainStart, cursor)
    ...
    const ordered = chain.includes(".order(") || ORDERING_HELPERS.some((helper) => wrapper.includes(helper))
...
test("every paged read in the projection path imposes a total order", () => {
  ...
  for (const file of [
    ...
    assert.deepEqual(pagedReadsWithoutOrder(file), [], `${file} pages a query with no .order()`)
  }
})
...
test("a fact payload hashes to the same value however its cost lines were paged", () => {
  ...
  assert.equal(booksDigest(sortFactCostLines(lines)), booksDigest(sortFactCostLines(shuffled)))

  // And through the real hashed payload, which is what the projector compares.
  const payload = (costLines) => hashableFactPayload({ memo: "Bill 1041", total_cents: 170000, cost_lines: costLines })
  assert.equal(
...
test("a vendor credit posts as the mirror of a bill instead of vanishing", () => {
  const amounts = (draft) =>
    Object.fromEntries(draft.lines.map((line) => [line.accountCode, line.debitCents > 0 ? line.debitCents : -line.creditCents]))
  ...
  const expenseCredit = draftFromFact({
    ...
    accountingDate: "2026-06-30",
    payload: { memo: "Returned materials", amount_cents: -3599, project_id: PROJECT_ID },
    sourceVersion: 1,
    projectionVersion: 1,
    policyVersion: 1,
  })
  ...
  const bill = postVendorBillFromCostLines({ ...common(), grossCents: 40000, costLines: [{ amountCents: 40000 }] })
  const netted = [...bill.lines, ...credit.lines].reduce((sum, line) => sum + line.debitCents - line.creditCents, 0)
  assert.equal(netted, 0)

  // Zero is still not a fact; a mis-signed retainage hold is still invalid.
  assert.throws(() => postVendorBillFromCostLines({ ...common(), grossCents: 0, costLines: [{ amountCents: 0 }] }), /must not be zero/)
  assert.throws(
    () => postVendorBillFromCostLines({ ...common(), grossCents: -40000, retainageCents: 4000, costLines: [{ amountCents: -40000 }] }),
    /gross and retainage are invalid/,
  )
  assert.throws(() => postExpense({ ...common(), amountCents: 0 }), /not an accounting fact/)
})
...
test("applying a vendor credit moves no cash, and an unreadable payment is still rejected", () => {
  ...
  assert.deepEqual(classifyPaymentPosting({ method: "credit", hasBill: true, hasInvoice: false, creditApplied: true }), {
    kind: "credit_application",
  })
  // The metadata flag alone is enough — the method column is nullable.
  assert.deepEqual(classifyPaymentPosting({ method: null, hasBill: true, hasInvoice: false, creditApplied: true }), {
    kind: "credit_application",
  })
  assert.deepEqual(classifyPaymentPosting({ method: "ach", hasBill: true, hasInvoice: false, creditApplied: false }), { kind: "bill_payment" })
  assert.deepEqual(classifyPaymentPosting({ method: "card", hasBill: false, hasInvoice: true, creditApplied: false }), { kind: "invoice_payment" })

  // Classifying explicitly must not become a licence to drop the unreadable ones.
  const orphan = classifyPaymentPosting({ method: "ach", hasBill: false, hasInvoice: false, creditApplied: false })
  assert.equal(orphan.kind, "unpostable")
  assert.match(orphan.reason, /neither a vendor bill nor an invoice/)
  const both = classifyPaymentPosting({ method: "ach", hasBill: true, hasInvoice: true, creditApplied: false })
  assert.equal(both.kind, "unpostable")
  assert.match(both.reason, /both a vendor bill and an invoice/)

  const projector = fs.readFileSync(path.join(__dirname, "../lib/services/books/projector.ts"), "utf8")
  assert.match(projector, /classifyPaymentPosting/)
  assert.match(projector, /if \(classification\.kind === "credit_application"\) continue/)
  assert.match(projector, /failures\.push\(\{ sourceType: "payment", sourceId: String\(row\.id\), error: classification\.reason \}\)/)
})

test("a customer receipt lands net of the fee that was withheld from it", () => {
  ...
  const receipt = postInvoicePayment({ ...common(), amountCents: 225000, feeCents: 6525 })
  assert.deepEqual(
    receipt.lines.map((line) => [line.accountCode, line.debitCents, line.creditCents]),
    [
      ["1000", 218475, 0],
      ["6050", 6525, 0],
      ["1100", 0, 225000],
  ...
  assert.equal(postInvoicePayment({ ...common(), amountCents: 225000 }).lines.length, 2)
  ...
  const drafted = draftFromFact({
    ...
    accountingDate: "2026-06-30",
    payload: { memo: "Receipt", amount_cents: 225000, fee_cents: 6525, project_id: PROJECT_ID },
    sourceVersion: 1,
    projectionVersion: 1,
    policyVersion: 1,
  })
  assert.deepEqual(drafted.lines.map((line) => line.accountCode), ["1000", "6050", "1100"])
  assert.throws(() => postInvoicePayment({ ...common(), amountCents: 1000, feeCents: 1000 }), /cannot equal or exceed/)
})

test("a source that leaves the projectable set is retired, and retirement is idempotent", () => {
  const facts = [
    { sourceType: "invoice", sourceId: "i-1", sourceVersion: 2, factKind: "invoice.recognized" },
    { sourceType: "invoice", sourceId: "i-2", sourceVersion: 1, factKind: "invoice.recognized" },
    { sourceType: "vendor_bill", sourceId: "b-1", sourceVersion: 1, factKind: "vendor_bill.recognized" },
    // Already retired on an earlier pass: must never be reversed a second time.
    { sourceType: "invoice", sourceId: "i-3", sourceVersion: 4, factKind: retiredFactKind("invoice") },
  ]
  // `i-1` is still billed; everything else has left.
  const live = new Set(["invoice:i-1"])
  assert.deepEqual(
    selectFactsToRetire(facts, live).map((fact) => `${fact.sourceType}:${fact.sourceId}`),
  ...
  const afterSweep = facts.map((fact) =>
    live.has(`${fact.sourceType}:${fact.sourceId}`) ? fact : { ...fact, sourceVersion: fact.sourceVersion + 1, factKind: retiredFactKind(fact.sourceType) },
  )
  ...
  assert.notEqual(booksDigest(retirementFactPayload(1)), booksDigest(retirementFactPayload(2)))
  assert.notEqual(
    booksDigest(hashableFactPayload(retirementFactPayload(1))),
    booksDigest(hashableFactPayload({ memo: "Invoice 12", total_cents: 225000, retainage_cents: 0, project_id: PROJECT_ID })),
  )
  ...
  const rebuild = fs.readFileSync(path.join(__dirname, "../lib/services/books/rebuild.ts"), "utf8")
  assert.match(rebuild, /isRetiredFactKind\(fact\.fact_kind\)/)
})

test("retirement runs only on a full pass, and reverses through the existing machinery", () => {
  const projector = fs.readFileSync(path.join(__dirname, "../lib/services/books/projector.ts"), "utf8")

  // On an incremental pass the candidate set is a watermarked slice, so "absent"
  // does not mean "gone" — retiring from it would reverse the entire ledger.
  assert.match(projector, /if \(options\.full\) \{[\s\S]{0,400}?retireDepartedSources\(orgId, liveSourceKeys, policyVersion\)/)
  assert.equal(projector.match(/retireDepartedSources\(/g).length, 2, "retirement has exactly one call site plus its definition")

  // One reversal path, not a second one invented for retirement.
  assert.match(projector, /reverseBooksJournalEntryForService\(\{[\s\S]{0,300}?reversalDate: fact\.row\.accounting_date/)
...
test("a half-finished supersede repairs itself on the next pass", () => {
  const projector = fs.readFileSync(path.join(__dirname, "../lib/services/books/projector.ts"), "utf8")

  // Reverse -> insert fact -> post is three writes and no transaction. Recovery
  // rests on four properties; each one is load-bearing and each is asserted here
  // because losing any of them leaves the ledger short an entry until someone
  // notices, which is exactly what "self-healing in theory" means.

  // 1. The reversal's posting key must be stable, so re-reversing is a no-op.
  //    It is derived from the candidate, never from the clock.
  assert.match(projector, /reason: `\$\{candidate\.sourceType\} was revised after posting`/)
  assert.match(projector, /reversalDate: candidate\.accountingDate/)

  // 2. Posting happens on every pass, not only when the fact was created — a
  //    crash between the insert and the post is otherwise permanent.
  assert.match(projector, /const journal = await postBooksJournalEntryForService\(draft, orgId, fact\.factId\)/)
  assert.doesNotMatch(projector, /if \(fact\.created\) \{?\s*const journal/)

  // 3. The watermark is inclusive, so a row whose `updated_at` equals it is
  //    revisited rather than skipped.
  assert.match(projector, /query\.gte\("updated_at", since\)/)
  assert.doesNotMatch(projector, /\.gt\("updated_at", since\)/)

  // 4. The nightly repair sweep really is a full pass.
  const maintenance = fs.readFileSync(path.join(__dirname, "../app/api/jobs/books-maintenance/route.ts"), "utf8")
...
test("an accounting date comes from the timestamp, not from the first ten characters of it", () => {
  ...
  assert.match(projector, /import \{ isoDateOnlyFromUtcMs \} from "@\/lib\/services\/reports\/dates"/)
  assert.equal(isoDateOnlyFromUtcMs(Date.parse("2026-06-30T23:30:00-05:00")), "2026-07-01")
  assert.equal(isoDateOnlyFromUtcMs(Date.parse("2026-06-30T23:30:00+00:00")), "2026-06-30")
...
test("revenue recognition is as-of, and names the projects it could not answer for", () => {
  const recognition = fs.readFileSync(path.join(__dirname, "../lib/services/books/revenue-recognition.ts"), "utf8")

  // Closing June in August must not book June revenue out of August's costs.
  assert.match(recognition, /const historical = periodEnd < todayIsoDateOnly\(\)/)
  assert.match(recognition, /loadPocPositionsAsOf\(\{ orgId, projectIds: pocProjects\.map\(\(row\) => row\.projectId\), asOf: periodEnd \}\)/)
  // Live computation survives only for a period end that is not yet in the past.
  assert.match(recognition, /\} else \{\s*const poc = await computeProjectPocForProject/)
  // A project with no snapshot is named, not given today's position.
  assert.match(recognition, /projectsWithoutSnapshot\.push\(project\.projectId\)/)
  assert.match(recognition, /skippedReason: "no_snapshot"/)
  assert.match(recognition, /return \{ periodEnd, basis, recognized, postedCents, projectsWithoutSnapshot, results \}/)
  ...
  assert.match(recognition, /if \(page\.length < RECOGNIZED_REVENUE_PAGE_SIZE\) break/)
  ...
  const poc = fs.readFileSync(path.join(__dirname, "../lib/services/poc.ts"), "utf8")
  ...
  for (const file of ["../lib/services/reports/wip-over-under.ts", "../lib/services/dashboard.ts"]) {
    const source = withoutComments(fs.readFileSync(path.join(__dirname, file), "utf8"))
    assert.match(source, /orderPocSnapshotsLatestFirst\(/, `${file} must read snapshots through the shared ordering`)
    assert.doesNotMatch(source, /\.order\("as_of"/, `${file} must not re-declare the snapshot ordering`)
  }
})
...
test("statement lines are paged, so the trial balance cannot stop summing", () => {
  ...
  assert.match(statements, /\.in\("entry_id", ids\)[\s\S]{0,200}?\.range\(from, from \+ 999\)/)
...
test("posting rules have no caller-less parallel path into the ledger", () => {
  const rules = fs.readFileSync(path.join(__dirname, "../lib/services/books/posting-rules.ts"), "utf8")
  ...
  for (const dead of ["postCustomerDeposit", "postDrawFunding", "postLoanPayment", "postOwnerActivity"]) {
    assert.doesNotMatch(rules, new RegExp(`export function ${dead}\\b`), `${dead} is a posting path with no caller`)
  }
})
...
function finding(overrides = {}) {
  return { category: "sync_error", entityType: "vendor_bill", entityId: "bill-1", details: {}, ...overrides }
}
...
test("a reconciliation finding that no longer reproduces is resolved, not left open forever", () => {
  ...
  assert.equal(recurring.newFindingCount, 0, "a finding already in front of somebody is not new")
  ...
  const deduped = planReconciliationItemSync({
    findings: [finding()],
    existing: [persisted({ id: "run-1" }), persisted({ id: "run-2" }), persisted({ id: "run-3" })],
    ownsCategory: ownsEverything,
  })
...
test("a machine-resolved finding reopens when it comes back; an accepted difference does not", () => {
  ...
  const accepted = planReconciliationItemSync({
    ...
    existing: [persisted({ id: "accepted", status: "explained", differenceCents: -2500 })],
    ownsCategory: ownsEverything,
  })
  assert.deepEqual(
  ...
  assert.equal(moved.newFindingCount, 1, "a changed accepted difference is worth a notification")
...
test("each sweep resolves only the categories it can reproduce", () => {
  ...
  const ownsSpineCategories = (category) => !PROJECTION_ITEM_CATEGORIES.includes(category)
  const plan = planReconciliationItemSync({
    findings: [finding()],
    existing: [
      persisted({ id: "spine", status: "open" }),
      persisted({ id: "projection", category: "projection_blocked_by_closed_period", entityId: "bill-9" }),
    ],
    ownsCategory: ownsSpineCategories,
  })
  assert.deepEqual(plan.resolveIds, [], "the other producer's item is untouched")
  assert.deepEqual(plan.carryForward.map((row) => row.id), ["spine"])
  ...
  const projectionPass = planReconciliationItemSync({
    ...
    existing: [
      persisted({ id: "spine", status: "open" }),
      persisted({ id: "projection", category: "projection_failed", entityId: "bill-9" }),
    ],
...
test("two exceptions of one kind against one record stay two findings", () => {
  ...
  const plan = planReconciliationItemSync({
    findings: [
      finding({ category: "retainage_mismatch", entityId: "project-1", findingKey: "retainage-orphan-project-1" }),
      finding({ category: "retainage_mismatch", entityId: "project-1", findingKey: "retainage-release-missing-project-1" }),
    ],
    existing: [],
    ownsCategory: ownsEverything,
  })
...
test("the close gate reads the current state of reconciliation, not its history", () => {
  const source = fs.readFileSync(path.join(__dirname, "../lib/services/books/period-close.ts"), "utf8")
  const spine = fs.readFileSync(path.join(__dirname, "../lib/services/books/reconciliation.ts"), "utf8")
  ...
  const gate = source.slice(source.indexOf('.from("accounting_reconciliation_items")'))
  assert.match(gate.slice(0, 400), /\.eq\("status", "open"\)/)
  assert.match(gate.slice(0, 400), /\.not\("category", "in"/, "tie-out categories stay out of the catch-all gate")
  ...
  assert.doesNotMatch(spine, /accounting_reconciliation_items"\)\s*\n\s*\.delete\(\)/)
  assert.match(spine, /status: "resolved"/)
  assert.match(spine, /resolved_by: null/, "the sweep is not a person; that is what tells them apart")
})
...
test("a truncated scan can never report a clean bill of health", () => {
  ...
  assert.equal(reconciliationRunStatus({ itemCount: 0, projectsSkipped: 0 }), "passed")
  assert.equal(reconciliationRunStatus({ itemCount: 0, projectsSkipped: 150 }), "warning")
  assert.equal(reconciliationRunStatus({ itemCount: 3, projectsSkipped: 0 }), "warning")
  ...
  for (const table of ["bank_transactions", "vendor_bills", "poc_snapshots", "bank_accounts", "companies", "bank_reconciliations"]) {
    const query = source.slice(source.indexOf(`.from("${table}")`))
    assert.ok(source.includes(`.from("${table}")`), `${table} is still scanned`)
    assert.match(query.slice(0, 600), /\.range\(from, to\)/, `${table} is paged`)
    assert.match(query.slice(0, 600), /\.order\("created_at"/, `${table} pages in a deterministic order`)
  }
  ...
  assert.equal(capped.length, 3, "sync backlog, waiver holds and retainage movement")
  assert.match(source, /if \(args\.issueCount > 0 \|\| args\.scanCapped\) return args\.failedStatus/)
})
...
test("the tie-outs compare real balances, including credit ones", () => {
  ...
  assert.equal(sumArSubledgerCents([{ balance_due_cents: 250_000 }, { balance_due_cents: -40_000 }]), 210_000)
  ...
  assert.equal(sumApRetainageCents([{ retainage_cents: 12_500, retainage_released_cents: 20_000 }]), -7_500)
  assert.equal(sumApRetainageCents([{ retainage_cents: 12_500, retainage_released_cents: 2_500 }]), 10_000)

  const rules = fs.readFileSync(path.join(__dirname, "../lib/services/books/tie-out-rules.ts"), "utf8")
  assert.doesNotMatch(rules, /Math\.max\(0/, "a clamp here is a permanent red on a legitimate balance")
})
...
test("retainage held on the schedule of values is tied to the retainage ledger", () => {
  ...
  const drifted = planRetainageControlFindings({
    sovLines: [
      { contract_id: "c-1", project_id: "p-1", retainage_held_cents: 500_000, retainage_released_cents: 0 },
      { contract_id: "c-1", project_id: "p-1", retainage_held_cents: 0, retainage_released_cents: 0 },
    ],
  ...
  assert.deepEqual(
    planRetainageControlFindings({
      sovLines: [{ contract_id: "c-2", project_id: "p-2", retainage_held_cents: 800_000, retainage_released_cents: 800_000 }],
      heldRetainage: [],
    }),
    [],
  )
  ...
  assert.deepEqual(
    planRetainageControlFindings({ sovLines: [], heldRetainage: [{ contract_id: "c-3", amount_cents: 40_000 }] }),
    [],
  )
})
...
test("a projection failure a retry cannot cure is surfaced as its own finding", () => {
  ...
  assert.equal(
    classifyProjectionFailure('Failed to record accounting fact: duplicate key value violates unique constraint'),
    "projection_failed",
  )
  ...
  const maintenance = fs.readFileSync(path.join(__dirname, "../app/api/jobs/books-maintenance/route.ts"), "utf8")
  const projection = fs.readFileSync(path.join(__dirname, "../app/api/jobs/books-projection/route.ts"), "utf8")
  const convention = /ok: failures === 0[\s\S]*status: failures === 0 \? 200 : 207/
...
test("a refused close leaves no journal entries behind, and posts under resolved versions", () => {
  ...
  assert.ok(
    close.indexOf("runBooksCloseChecklist") < close.indexOf("recognizeRevenueForPeriod"),
    "the checklist gates recognition, not the other way round",
  )
  assert.ok(close.indexOf("blockingFailures.length > 0") < close.indexOf("recognizeRevenueForPeriod"))
  ...
  assert.match(close, /recognizeRevenueForPeriod\(context\.orgId, period\.period_end\)/)
  ...
  const yearEnd = source.slice(source.indexOf("export async function closeFiscalYearToRetainedEarnings"))
